...
Fill in or remove the optional sections (each marked with an `OPTIONAL:` HTML comment above the heading).
...
stream — there is no monolithic mirror; do not recreate one. Flat-file
per-entry is the sole supported mode.
