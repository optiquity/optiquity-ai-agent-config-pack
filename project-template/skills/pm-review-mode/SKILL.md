---
name: pm-review-mode
description: Set the review mode — how you are asked to respond to the open items agents surface (itemized, full, hybrid, or none). Run to change how open-item review is presented for this session.
allowed-tools: Bash
---

Set the session's `review_mode`. This is a PM-chat-only selector; it writes the
...
authorizes PM-chat auto-commit (never auto-push; agents never commit). Require
...
the selected `review_mode` value so its active behavior is explicit in context.
Point to `/pm-help` for the per-mode detail.
