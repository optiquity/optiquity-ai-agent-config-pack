---
name: pm-intervention-mode
description: Set the intervention mode — how much the PM chat pauses at its gates (full, pre-coder, ambiguity, or none). Run to change how often the workflow pauses for your input this session.
allowed-tools: Bash
---

Set the session's `intervention_mode`. This is a PM-chat-only selector; it
writes the choice to the per-clone config and takes effect at the next gate. See
`docs/pack/PM-OPERATING-MODES.md` for the full behavior of each value.

**Commit-gate enforcement is Claude-only.** The pause gates are honored by the PM
chat on every CLI; only the commit-approval gate's mechanical backstop (a fresh
per-commit token) is Claude-only hook-enforced.
...
PM-chat auto-commit (never auto-push; agents never commit). Require an explicit
