...
16. **auditor-code** — language-specific code quality across five audit dimensions: (a) idiom adherence, (b) dead code and unused imports, (c) performance anti-patterns (N+1, blocking main thread, unnecessary allocations in hot paths), (d) concurrency safety (race conditions, missing async handling, incorrect isolation annotations), and (e) systemic error handling (boundary mapping consistency, retry policy uniformity). The boundaries auditor-code audits for mapping consistency are exactly those defined in `error-handling` rule 4 (repository / service / external-API ingress) plus every transport the project uses per `grpc-patterns`, `rest-patterns`, or other loaded protocol skills. A project with multiple transports (e.g., gRPC, REST, message queue) must show consistent mapping across all of them. Per-function error-handling defects (empty catch blocks, swallowed errors, error types that lose context, missing re-raise after log) are language-idiom findings unless they recur at 3+ sites; tag them `[per-function — reviewer]` in the error-handling skill. **Boundary with auditor-architecture.** Rule 16 covers error-handling *patterns* at and across boundaries (retry policy uniformity, boundary mapping consistency, exception type discipline, taxonomy alignment); auditor-architecture covers error-flow *architecture* (whether the error-handling layer is correctly placed in the call stack, whether the boundary itself exists at the right seam, whether the abstraction shape is right). Per rule 35, architecture wins over code idiom when a finding is layer-shaped. **Systemic threshold (named test).** A finding is *systemic* when the same divergence or omission appears at three or more independent call sites, OR crosses module boundaries (the same defect in two different services / packages / modules). A single-site instance belongs to per-PR review (the `reviewer` agent), not to auditor-code. When the threshold is met, file once as a systemic finding listing all affected sites, not N separate per-site findings. **Independent call site** is defined as a distinct file:symbol pair where the defect is materially decided — that is, the location where fixing the code resolves the defect at that site. A shared helper called from N callers counts as **1 site at the helper** when the defect lives in the helper (fixing the helper fixes all callers); the same defect copy-pasted into N sibling modules with byte-identical implementations counts as **N sites** because each module owns an independently-fixable copy. Multiple calls from the same module or compilation unit count as one site. **Worked examples:**
    - *Scenario A — single-site empty `except`.* One Python repository class has an empty `except Exception: pass`. Sites: 1. Cross-module: no. **Call:** per-function defect, out of scope for auditor-code; routes to `reviewer` per `error-handling` rule 5 `[per-function — reviewer]`.
    - *Scenario B — same empty-`except` pattern in 3 sibling repositories.* The defect appears in `user_repository.py`, `order_repository.py`, and `inventory_repository.py` (3 distinct symbols, 3 distinct files). Sites: 3. **Call:** SYSTEMIC under the count threshold (3 ≥ 3). File one Major finding listing all three sites; recommend a shared error-mapping convention at the repository base class.
    - *Scenario C — cross-transport mapping divergence.* The gRPC adapter maps `Status.UNAVAILABLE` to `DomainError.serviceDown`; the REST adapter maps the equivalent HTTP 503 to `DomainError.unknown`. Sites: 2, but in different transport packages. **Call:** SYSTEMIC under the cross-module clause (count below 3 is irrelevant when the cross-module trigger fires). File one Major finding citing both adapters; recommend unifying the retryable-vs-non-retryable taxonomy across transports.
...
29. **auditor-docs** — scope: `**/*.md`, `**/*.txt`, `**/README*`, inline doc comments (`///`, `"""..."""`, `/** ... */`). Cross-references documented claims against code in auditor-architecture's and auditor-code's scope but does not re-audit those files for anything other than documentation accuracy.
    - **Per-entry source-of-truth trees are IN SCOPE.** Per-entry tree files (`docs/project/backlog/TD-NNN.md`, `docs/project/implementation-plan/phase-N.md`, `docs/project/changelog/YYYY-MM-DD-*.md`, including each stream's `_rules.md`, `_intro.md`, and `_toc.md` supporting files) are authored source-of-truth in flat-file mode and are audited as documentation per the rule above. Same applies to the pack-side per-entry trees at `/backlog/` and `/changelog/` when present (pack-self dog-food).
    - **Pre-v11.0 monolithic files are a conversion INPUT, not an audit target alongside the tree.** In flat-file mode the per-entry tree is the sole source of truth — there is no regenerated monolithic mirror. A stream's per-entry tree is "present" when its directory exists and contains `_rules.md` (aligned with `validate-pack.py` Checks 32/33/34 detection — no entry-file requirement; a greenfield stream installed by `init-project.sh` with `_rules.md`/`_intro.md`/`_toc.md` but zero entry files yet still counts as present). If only a pre-v11.0 monolithic `BACKLOG.md` / `IMPLEMENTATION-PLAN.md` / `CHANGELOG.md` exists (no decomposition applied yet), audit that monolithic file as before — it is the only source until conversion runs.
...
44. **Skip `auditor-ui`** when the project has no UI layer. Detection: no `*.xcodeproj`, no SwiftUI/UIKit/AppKit source files, no `**/*View.swift` files, no frontend framework manifests. Pure backend / CLI / server projects skip this cluster. *Note:* the current detection list is Apple-centric.
...
58. **Antigravity CLI (`agy`)** — Antigravity runs this project's agent roster from the plugin bundle at `.agents-plugin/optiquity-agents/` (`agy plugin install ./.agents-plugin/optiquity-agents`), or, where the plugin `agents/` template schema is not yet accepted, from the runtime `define_subagent` / `invoke_subagent` pattern (see `.agents-plugin/optiquity-agents/RUNTIME-SUBAGENT-PATTERN.md`). For the auditor, `agent-run.sh` provides external orchestration on the `agy` branch: runs each non-skipped subagent in its own `agy` headless (`agy -p`) session, captures reports to temp files, then invokes the auditor parent with all reports as input via stdin (not inline — see rule 59). The bundle agent files provide each subagent's system prompt, scope, and skill instructions; the script prompt adds project-specific context. <!-- RE-VERIFY at impl: agy subagent activation + headless --print semantics, antigravity.google/docs/subagents -->
...
60. **Skip-rule passing.** Skip decisions originate with the PM chat or the developer. Claude and Codex parents receive skip rules as prose in their invocation prompt ("Skip auditor-ui and auditor-tests for this server-only project"). Antigravity receives skip rules via the `agent-run.sh --skip` flag. All three mechanisms produce the same effect.

## Cost and parallelism

61. A full audit is expensive: 7 subagent invocations (or 8 counting the parent) per run. Parallelize where possible — Claude's Task tool and Codex's `max_depth=2` can run subagents concurrently; Antigravity's external orchestration runs sequentially for log readability and to avoid rate limits.
...
66. **Choose per-tool spawning.** For Claude use the Task tool with parallel calls; for Codex use `max_depth` in `config.toml`; for Antigravity (`agy`) use the plugin bundle (`.agents-plugin/optiquity-agents/`) or the runtime `define_subagent` / `invoke_subagent` pattern from the main session, or `agent-run.sh` external orchestration for headless (`agy -p`) execution.
...
68. When the auditor finishes, the consolidated report is returned to the developer or PM chat. The auditor does not write to the backlog tree (`docs/project/backlog/`), `STATUS.md`, or any other project file.
...
70. The developer may re-run a single subagent (e.g., `auditor-security` after fixing a CVE) to verify the fix without running a full audit. Use the per-subagent invocation paths: Claude `--agent auditor-security`, Codex `--agent auditor-security`, Antigravity `./agent-run.sh agy --agent auditor-security`.
