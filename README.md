...
|---------|--------------|---------------|
| v11.0 (RC2) | September 2026 | Issue-tracker integration **deferred (dormant) — BD-214** (Phase A — D-1..D-23 surfacing as forced trinity addenda + per-CLI `/pm-help` (renamed from `/pack-help` by BD-257) + HELP-FRAGMENT shared content; Phase B — tracker abstraction with `gh` backend, forward/reverse migration, recommendation system — code retained dormant, flip blocked); flat-file per-entry is the sole supported mode; BD-088 customization-preservation library + truthful report (BD-059 fix); migrate-v10-to-v11.sh + BD-119 N→N+1 migrator framework + BD-147 reusable migrator-skills.sh + BD-144 capability-translation stage; init-project.sh `--update`; **skill-dimensions reframe (BD-141..BD-150 + BD-156..BD-159)** — 5 dimensions D1–D5 + Tier 0 base + intersection + trigger-loaded model in PLATFORM-SKILLS.md (BD-142); trinity Skill-loading prose realigned (BD-143); add-capability.sh D5 rename + intersection fix (BD-144); init-project.sh D1/D5 detection hint + python-data marker (BD-141, BD-145); 3 new `*-patterns` skills — protobuf-patterns (BD-156), apple-swiftdata-patterns (BD-157), swift-concurrency-patterns (BD-158); naming-convention codification + maintainability principle in pack memory (BD-149, BD-159); MIGRATION-v10-to-v11.md + MERGE-STRATEGY.md skill-model behavioral note (BD-148); validate-pack.py expanded to 93 invoked checks (88 numbered Check 1–11, 16–20, 22–23, 25–27, 29–75, 77–94, 97–98, with Checks 16/18/19 each registering twice and 2 unnumbered entries for 93 registry entries total — including DEEP-only Check 49; the registry is the source of truth — see _build_check_registry(); gaps reflect retirements: Checks 12–15 retired per v9 sunset, Check 24 retired per BD-194, Checks 21 + 28 retired per BD-221, Check 76 retired per BD-253; Checks 95–96 reserved for BD-289, unlanded) — per-CLI parity, help-fragment freshness/completeness, customization regression guard, BD-146 Check 31 skill-cell internal-consistency gate, BD-168 Checks 32/33/34 per-entry split validators (mirror-in-sync, TOC-in-sync, cross-reference integrity), BD-175 Checks 36/37/38 pack/project boundary (commit-scope honesty, project-side pack-only deny-list, pack-only-file siting) + Check 39 cmd_update mapping/glob symmetry, BD-179 Check 40 pack-ops/ bare cross-reference scanner, BD-180 Check 41 `_CLIENT_INSTALLED_FILES` self-doc list integrity, BD-184 Check 42 CI workflow wires all per-check test files, BD-173 H.14 Check 43 project-side bare cross-reference scanner (V11 leak-sweep prevention); aggregate CI test runner across the wired test suites; **RC2 (BD-291 + BD-293)** — migration delivers every path the install map declares (agent-run.sh migrate token, never-existed-file baseline, retired-skill-dir removal, Check 97 install-map axis symmetry); `--update` no longer reverts client work and now carries the session-config ignore entry, narrowed to the two runtime-state keys so a pruned pack entry stays pruned; pause/report sites name the pack path of a skill that installs later; `/pm-startup` reports the commit gate's EFFECTIVE state with a multi-leg canary and two wired canary tests; the RAG listing is scoped and names real tool verbs; report writes sanction a Bash heredoc alongside the Write tool across the 63 agent definitions |
| v10.1   | May 8, 2026  | Per-agent permission profiles across all 48 project-template agent files (read-only / write-scoped / write-script / orchestrator); project trinity (CLAUDE.md / AGENTS.md / GEMINI.md) gains Project memory section; PM-CHAT.md gains Permission profiles section + profile-assignment table + tightened prompt-construction rules; RAG ingestion manifest + RAG index hygiene principle in METHODOLOGY.md; /pm-startup Step 4 expanded to full RAG reconciliation with `RAG:` summary line; migrate-v9-to-v10.sh emits RAG sync section; Procedure 5-S Task C confirms /pm-startup reconciliation post-migration; validate-pack.py Check 21 enforces per-agent canonical-phrase compliance |
...
├── .claude/skills/pm-help/                 Claude pm-help skill (v11; invokes pm-help.sh)
├── .codex/skills/pm-help/                  Codex pm-help skill (v11)
├── .agents/skills/pm-help/                 Antigravity pm-help skill (v11)
...
│   ├── HELP-FRAGMENT.md                    Per-project verb reference (v11; rendered by pm-help.sh)
...
├── archive/v11/                            Extracted operating-doc history + superseded design/research records
...
├── validate-pack.py                        CI structural validation (93 invoked checks — 88 numbered Check 1–11, 16–20, 22–23, 25–27, 29–75, 77–94, 97–98, with Checks 16/18/19 each registering twice and 2 unnumbered entries for 93 registry entries total — including DEEP-only Check 49; registry is the source of truth (_build_check_registry()); gaps reflect retirements: Checks 12–15 retired per v9 sunset, Check 24 retired per BD-194, Checks 21 + 28 retired per BD-221, Check 76 retired per BD-253; Checks 95–96 reserved for BD-289, unlanded; pack-internal)
...
└── lib/                                    Shared bash libraries
    ...
    ├── marker-preserve.sh                  BD-136 trinity marker-aware merge engine (v11)
    ...
    ├── tracker-{config,doctor,init,labels,errors,sidecar,mirror,agent-read,phase-task,cycle-check,links}.sh   Tracker subsystem (v11; phase-task per V3.3 §2 D-21 / BD-106; cycle-check + links per V3.3 §5 / BD-108; doctor per BD-130; dormant, deferred per BD-214)
...
scripts/tests/fixture-dependent/test-migration-delivers-install-map.sh  BD-293 acceptance sweep — every declared install-map path delivered after a v10→v11 migration (v11; fixture-dependent, BD-219)
