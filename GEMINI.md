...
- **Reconciliation-instance independence.** A reconciliation pass (the round
  ...
  re-engage an existing agent (on Antigravity via the platform's known-ID
  re-engage / idle-rewake path); (2) **architect challenge** — a good,
  evidence- and logic-based reason argued per case (not a blanket exemption).
  REINFORCES `fresh-agent-default` (the independence principle applied to
  reconciliation) and SUBORDINATES the Agent-team "SendMessage for follow-ups"
  convenience: a reconciliation pass is a fresh spawn unless a carve-out fires.
  `[roles: universal] [rationale: reconciliation-instance-independence]`
...
- **Uniquely + descriptively name every spawn.** Every spawned agent carries a
  ...
  cycle (uniqueness is a DISCIPLINE — no platform guarantees it). On Antigravity address
  by the known agent ID / named-role type; on Claude Code / Codex use the platform's
  agent-name field. A
