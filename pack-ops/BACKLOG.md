...
Unblocks: the pack's backlog + changelog become per-entry DIRECTORY trees (`/backlog/`, `/changelog/`) as the SOLE SSOT; the monolithic `pack-ops/BACKLOG.md` + `CHANGELOG.md` are DELETED (NO regenerated mirror). BD-204 (GH Issues) builds on it.
...
Binding decisions (user 2026-06-04 — FIXED; the fresh adversarial architect designs WITHIN them and CHALLENGES every other choice the first design made):
  - DELETE the monolith: `pack-ops/BACKLOG.md` + `CHANGELOG.md` are DELETED entirely — NO regenerated mirror, NO kept copy. The per-entry tree is the SOLE SSOT. OVERRIDES the "per-entry trees vs mirrors" keep-a-mirror convention FOR THE PACK. Rationale: agents repeatedly source from stale/wrong primaries; deleting the monolith forces every dangling reference to BREAK + SURFACE + be fixed, rather than silently sourcing stale data. Breaking is the CORRECT, desired outcome. There must be NO case in which the monolith is useful.
  - ARCHIVE history OUT of agent-read surfaces: historical/archival content (old CHANGELOG versions, resolved-table bloat, intra-section prose) is BLOAT and must NOT appear in any agent-read surface (the active per-entry tree, governance docs). Archive it OUT (preserved inert, never agent-read). Active surfaces carry LIVE content only. NOT byte-identical preservation (the first design's wrong target).
  - SAFE before DELETE: confirm the per-entry tree captures every LIVE entry content-faithfully (no live BD/version lost) BEFORE deleting the monolith (the destructive step, gated on verification). "Lossless" applies to LIVE entries only; history is archived, not preserved as bloat.
  - The deletion FORCES reference fixes: removing the monolith breaks every `CLAUDE.md`/README/`PACK-AGENTS.md`/`PACK-CHAT.md`/validator (Check 32) reference that names `pack-ops/BACKLOG.md` as a mirror. Those breaks MUST surface + be FIXED (repointed to the per-entry tree) as part of BD-203 so CI is green with NO monolith. This supersedes the earlier "don't pre-edit the trinity" (which was about pre-emptively papering the doc-gap; now the deletion forces the REAL fixes).
  - pack-only — zero `project-template/` or client-side change.
Scope: create `/backlog/` + `/changelog/` per-entry SSOT trees from the current monolithic LIVE content; archive the historical/bloat content OUT of agent-read surfaces; DELETE the monolith flat files (no mirror); fix every dangling reference the deletion surfaces (validators incl. Check 32, governance/doc references) so validate-pack is GREEN with the monolith gone + the per-entry tree the sole SSOT; the `_rules.md` per-stream contracts (per-entry-SSOT, NO mirror).
Out of scope: GH Issues / tracker Mode 3 (→ BD-204); any implementation-plan stream (the pack has none).
Acceptance criteria (END-OF-BD FULL CORRECTNESS AUDIT): `/backlog/` + `/changelog/` are the SOLE per-entry SSOT; every LIVE entry is content-faithful (no live BD/version lost) — verified BEFORE deletion; historical content archived OUT of agent-read surfaces (active tree = live content only, no bloat); the monolith flat files DELETED (grep proves no surviving `pack-ops/BACKLOG.md`/`CHANGELOG.md`); every dangling reference fixed (validate-pack GREEN with no monolith; no surviving reference sources the deleted files); a full integrated correctness audit confirms no live data lost + no stale-source path remains + reality matches the docs.
