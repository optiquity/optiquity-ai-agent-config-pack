...
Status: Deferred
Blockers: ALL phases (P1 brainstorm/research through P2/P3 editing) wait until BD-195 is complete — not split, not started early. Even the audit/research needs a stable tree: BD-195's S2/S3/S4 churn the same files (CLAUDE.md ## Pack memory, maintenance-docs/ plans, commit-discipline mirrors, pack-ops/) and S4 may relocate or add worktree content. Timing after BD-195 is the user's call — possibly directly after, possibly after further BD batches (other work may be more urgent). The user moves this to Open/Unblocked when ready.
...
Type: feat — STRUCTURAL. Spans client-shipped scripts, the conditional-file distribution mechanism, a capability-table single-source refactor, the `pack update` propagation path, and multiple client docs (METHODOLOGY Procedure 6, HELP-FRAGMENT, PM-CHAT, INSTALL-PROCEDURES). Requires the full pipeline (first architect → ADVERSARIAL fresh-architect review → planner → coder → bounded review/fix). No scope keyword on the entry; phase commits declare their own.
Status: Open
Blockers: Follows BD-195 (Resolved 2026-06-03 — BD-195 handed off all capability-addition findings here). An ADVERSARIAL fresh-architect review (challenging every choice in the first design `ARCHITECTURE-BD-200.md`, given the user decisions below) precedes planning, per user direction 2026-06-04. The first design's gitignored-pool choice is a KNOWN GAP corrected below.
Unblocks: a client project can ACTIVATE a pack-supported capability (a D1–D5 dimension — platform / language / protocol / deployment surface, e.g. add Python or a server component to an existing Swift/iOS project) WITHOUT a pack-repo clone, on ANY clone of the project. Precedes v11.0 launch (launch gate: S1 ∧ S2 ∧ S3 ∧ S4 ∧ BD-195 ∧ **BD-200** ∧ BD-185).
...
Binding design decisions (user, 2026-06-03/2026-06-04 — FIXED inputs to the adversarial architect; not for re-litigation, though the architect MUST surface any it finds unrealizable rather than silently deviate):
  ...
  - UPDATE correctness contract: `pack update` propagates add/delete/MODIFY of pack-provided masters to EVERY location the artifact lives — skills → three trinity dirs; active conditional files → live tree + pool; inactive conditional files → pool — via the standard update path, producing the SAME end-state a wipe-repopulate would. DELETE-propagation explicitly verified.
File/Symbol:
  ...
  - NEW tracked client-local conditional-file pool (population mechanism — copy-then-delete vs S9-move — for the architect to finalize given the tracked decision; user leans copy-then-delete).
  - `scripts/init-project.sh` `stage_s9_conditional_remove()` + the `pack update` path (`_cmd_update_iter_dir` / S-stages) — preserve masters into the pool (not destroy the only source); propagate add/delete/modify to skills (trinity dirs) + active conditional files (live tree + pool) + inactive conditional files (pool).
...
Description:
  ...
  **Scope:** project-side `activate-capability.sh` + the tracked conditional-file pool + the `stage_s9_conditional_remove()` interaction + the single-source capability-table refactor + Procedure 6 redesign + client-surface reference rework + the `pack update` propagation wiring with the delete-propagation contract.
  ...
  **Acceptance criteria:** a client can ACTIVATE a capability (e.g. add Python to a Swift-only project) with NO pack clone present, on a FRESH clone of the project; the conditional-file pool is tracked and travels with the repo; `pack update` keeps skills (trinity dirs) + active conditional files (live tree + pool) + inactive conditional files (pool) correct, INCLUDING deletes, with the same end-state as a wipe-repopulate; capability tables single-sourced; Procedure 6 + client-surface references describe the project-side mechanism (no "run from the pack", zero pack-self tokens — Check 43 clean); `validate-pack` green; dependency-direction rule + Check 47 honored (no allowlist growth).
  **References:** BD-195 completeness re-audit (2026-06-03 — the `add-capability` cross-surface finding); `ARCHITECTURE-BD-195-ADD-CAPABILITY-SHIPPING.md` (the verdict that triggered this); `ARCHITECTURE-BD-200.md` (first architect design — under adversarial review; gitignored-pool choice is a KNOWN GAP corrected here); user design decisions 2026-06-03 / 2026-06-04 (OQ-1/2/3 + tracked pool + single-source + update contract); `dependency-direction-placement` (trinity `## Pack memory`).
Resolved: n/a
