...
Fork + BD-185 sequencing (DECIDED 2026-06-04, `DECISION-PER-ENTRY-FORK-AND-BD185-SEQUENCING.md`): co-design the SHARED TOOLING once (pack+project — the per-entry engine `_lib.sh` + the tracker reverse emitter are ONE codebase across all streams); design the DOCS/ASSET corrections per-side (48 pack-actionable vs 16 project surfaces, disjoint, separate artifacts). BD-185 runs AFTER BD-203 (a CONSUMER of the per-entry/tracker substrate, NOT a prerequisite). Next: the implementation-design architect pass (shared-tooling co-design + pack conversion).
Status: Open
