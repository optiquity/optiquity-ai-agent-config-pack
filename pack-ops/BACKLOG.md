...
**BD-197 — Remove all Claude Code worktree-isolation references from pack and project content**
Type: fix — content cleanup; remove opinion-bearing references to Claude Code Agent-tool worktree isolation (v11-dev; re-audit-driven). No scope keyword asserted here — Phase 2's actual diff determines whether the implementing commit is pack-only or mixed.
Status: Deferred
Blockers: Queued, not actionable now. Implement at a clean breaking point in the BD-195 / BD-196 work line; the user moves this to Open/Unblocked when ready. (This is intra-v11.0 sequencing, NOT a v12 deferral — despite the shared `Deferred` status value, which is the pack's only queued-not-actionable convention. v11-dev edits reach main at the v11.0 merge.)
Unblocks: Pack and project content becomes fully agnostic on Claude Code Agent-tool worktree isolation — no prohibition, requirement, recommendation, description, or hint in any direction — so each developer's own `~/.claude/settings.json` choice (e.g., `"worktree": { "baseRef": "head" }`) is unconstrained by pack content.
File/Symbol: Starting-point locations from the 2026-05-31 inventory (line numbers WILL drift; Phase 1 rebuilds the list fresh and confirms each location's scope before excision):
  - `CLAUDE.md` — the worktree-isolation bullet inside the `### Sub-agent behavior (Claude-only)` subsection of `## Pack memory` (~L319-330 on 2026-05-31). Remove ONLY that bullet; preserve the subsection header and its other bullets ("Default sub-agent spawns to background", "Agent-team stage lifecycle + per-commit fresh-coder"); trim the shared Trinity-exemption note so it no longer references the removed rule.
  - `pack-ops/CONCEPTUAL-REVIEW-METHODOLOGY.md` (~L194) — derivative prohibition; surgically excise only the isolation portion if the line bundles other guidance.
  - `maintenance-docs/v11-implementation/EXECUTION-PLAN-V11.0.md` §D (~L415-419) — confirm §D is exclusively isolation content before whole-section removal; otherwise excise only the isolation portion.
  - `maintenance-docs/v11-implementation/PLAN-SKILL-DIMENSIONS.md` §4.8 (~L923-1294) — 5 spawn-prompt reproductions; confirm §4.8 is exclusively isolation content before whole-section removal; otherwise excise only the isolation reproductions.
  - `maintenance-docs/v11-implementation/PLAN-DOC-CONCISION-GUARDRAILS.md` (~L187) — remove the isolation reference.
  - `maintenance-docs/v11-implementation/ARCHITECTURE-BD-196-S1-CORPUS-CLASSIFICATION.md` (~L88) — remove the classification row for the prohibition rule entirely (it also cites a now-stale line range); do not update it — the underlying rule is gone.
  - `.claude/skills/commit-discipline/SKILL.md` + `.codex` + `.gemini` mirrors — strip ALL worktree vocabulary and isolation-coupled operational mechanics (no `worktree-agent-*` patterns, no `.claude/worktrees/...` paths, no "main checkout vs. worktree" framing, no mention of isolation as a concept). Restructure §1 pre-flight, §2 write-target example, and §6 anti-patterns to generic working-tree language (verify pwd / HEAD match the spawning prompt; "writes go under pwd; writes elsewhere are wrong"; "/tmp retries → wrong path" without worktree framing). The BD-119 C-2 incident reference stays only if rephrasable as generic "wrong working tree"; otherwise excise. Edit the three mirrors in lockstep (skill-parity, not trinity).
  - 4 dangling references to the deleted memory file `feedback_worktree_isolation_broken_from_v11_clone.md` — excise each pointer (do not repoint; the entry is gone): `ARCHITECTURE-SKILL-AGENT-MAINTAINABILITY.md` (~L642-643), `RESEARCH-CLAUDE-REPOS-SURVEY.md` (~L159, ~L299), `RESEARCH-19C-G-ITEMS-VERIFICATIONS.md` (~L66).
  - 4 historical decision-record references — `ARCHITECTURE-CLEANUP-BATCH-19C-V2.md`, `ARCHITECTURE-PRE-19C-SALVAGEABILITY.md`, `RESEARCH-19C-G-ITEMS-VERIFICATIONS.md` (separate from the dangling-ref hit): per audit, either leave as clearly past-tense historical record with no current-rule force, or excise; do NOT update to reflect any new position.
Description:
  **Problem:** Pack and project content currently carries opinion-bearing content about Claude Code Agent-tool worktree isolation: a prohibition rule in `CLAUDE.md ## Pack memory`, reproductions across multiple v11-dev planning/skill docs (the 2026-05-31 inventory found ~15 live carriers; Phase 1 refreshes the exact set), and assumptions in the `commit-discipline` skill mirrors (which currently assume isolation IS active). The original reason — an Anthropic-side bug that created isolated worktrees at `origin/main` HEAD regardless of the parent chat's branch — now has a developer-side workaround (`"worktree": { "baseRef": "head" }` in `~/.claude/settings.json`). Opinions about isolation in pack/project content are noise at best and obstruct a developer's own settings choice at worst.
  **Position — pure removal:** No replacement, no softening, no rewording, no inversion. After this work, pack and project content makes NO mention of Claude Code Agent-tool worktree isolation in any direction — no prohibition, requirement, recommendation, description, or hint that isolation is a thing for developers to decide about. Where removal leaves text incoherent (notably the `commit-discipline` mirrors), restructure minimally to restore coherence using generic working-tree language — without introducing any new opinion about isolation or even a neutral mention of it as a concept.
  **Process note (Phase 2 implementer):** Removing the `CLAUDE.md ## Pack memory` content is a PM-chat / trinity-governed edit and MUST follow the ordered propagation procedure in `pack-ops/PACK-CHAT.md` — not treated as an ordinary doc edit.
  **Out of scope (explicit):** the `main` branch (v11-dev edits reach main at v11.0 merge); `~/.claude/settings.json` user-global (developer's responsibility); any `.claude/settings.json` / `settings.local.json` at any project level (pack repo or project-template — pack ships nothing about this); adding any new content about isolation in any direction (removal-only); any new `validate-pack.py` check related to isolation (none today, none added); any `project-template/.claude/settings.json` content (not shipped).
  **Phase 1 — Re-audit (dual lens):** Re-run a blast-radius inventory against v11-dev's then-current HEAD (prompt similar to the 2026-05-31 inventory in chat history). Catalog BOTH directions of opinion: content that PROHIBITS isolation (the rule + reproductions) AND content that ASSUMES isolation IS active (commit-discipline mirrors, anything similar). Catalog purely-neutral mentions separately so the sweep leaves neutral plumbing alone. For every cataloged location, confirm section/subsection boundaries before any wholesale section removal — surgically excise only the isolation content; never delete bundled adjacent rules or unrelated section content. Mandatory ripgrep discipline: `--hidden --no-ignore`. In scope for verification: pack-side, `project-template/` side, and `AGENTS.md` / `GEMINI.md` in both pack and project-template. Out of scope: main branch and user-global.
  **Phase 2 — Removal sweep:** Edit each cataloged location per the refreshed audit; remove the isolation content, do not replace. Restructure incoherent text (commit-discipline) minimally to generic working-tree language with no isolation vocabulary. Trinity-exempt — the pack-memory rule is Claude-only by design; removal does not propagate to `AGENTS.md` / `GEMINI.md` (nothing there to remove). The `commit-discipline` mirrors edit in lockstep across `.claude` / `.codex` / `.gemini` (skill-parity). The `CLAUDE.md ## Pack memory` edit follows the PM-chat / trinity procedure noted above.
  **Acceptance criteria:**
  - Zero pack-side content (`CLAUDE.md`, `pack-ops/`, `maintenance-docs/`, `scripts/`, `.claude/skills/`, `.claude/agents/`, `.codex/`, `.gemini/`, etc.) carries any opinion-bearing reference to Claude Code Agent-tool worktree isolation in either direction.
  - Zero `project-template/` content carries any such reference.
  - `commit-discipline/SKILL.md` ×3 mirrors contain no worktree vocabulary (no `worktree-agent-*`, no `.claude/worktrees/...` paths, no "main checkout vs. worktree" framing, no mention of isolation) and remain functional under generic working-tree assumptions.
  - All 4 dangling memory-pointer references are excised.
  - The stale line-number citation in `ARCHITECTURE-BD-196-S1-CORPUS-CLASSIFICATION.md` is removed (not updated — the rule is gone).
  - The 4 historical decision-records are dispositioned per audit.
  - Unrelated rules in the `### Sub-agent behavior (Claude-only)` subsection of `CLAUDE.md ## Pack memory` are preserved intact.
  - Unrelated content in `EXECUTION-PLAN-V11.0` §D and `PLAN-SKILL-DIMENSIONS` §4.8 (if any) is preserved intact.
  - `scripts/validate-pack.py` passes.
  - No edits anywhere outside v11-dev; no new opinion-bearing content; no new files shipped.
  **References:** Workaround verification performed in chat 2026-05-31 and 2026-06-01: three Agent-tool isolation probes — (1) initial bug confirmation (agent landed at `origin/main` HEAD `7ccbba9`, not the v11-dev parent); (2) `worktree.baseRef:"head"` at project-level `.claude/settings.local.json` (agent landed at parent HEAD `3178fa4`); (3) `worktree.baseRef:"head"` at user-global `~/.claude/settings.json` only, project-level reverted (agent again landed at parent HEAD `3178fa4`, `HAS_V11_RESEARCH: yes`) — confirming the user-global location alone suffices with no project-level fallback. Plus the 2026-05-31 blast-radius inventory, which is the source of the starting-point location list above and will be replaced by Phase 1's refreshed audit at implementation time.
Position: queued behind the BD-195 / BD-196 work line; the user moves it to Open/Unblocked at a clean breaking point.
Resolved: n/a
