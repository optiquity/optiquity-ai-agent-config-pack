...
| `bootstrap.sh` | `scripts/` | First-time setup — resolves SPM dependencies | Manual, once per machine |
| `format.sh` | `scripts/` | Format Swift (swift-format). Manual only — not in the auto-hook | `repo-ops` or manual pre-commit |
| `test.sh` | `scripts/` | Run test suite only | `repo-ops` or manual |
