...
- **Per-action approval extends to sub-agents.** The "no state-changing
  ...
  Codex CLI Pack Chat AND every sub-agent it spawns. State-changing
...
- **Pack-coder PREFLIGHT + STOP-MEANS-STOP pattern.** Every pack-coder
  ...
  - **STOP-MEANS-STOP preamble (REQUIRED for all CLIs as content;
    ...
    Stop authority is absolute and unconditional." Codex has no
    SendMessage equivalent (confirmed absent per issue #12462) and
    no documented transcript-classifier handoff-check equivalent of
    Claude Code's SECURITY WARNING; parent-side enforcement relies on
    the `/agent` slash command or natural-language directive ("ask
    Codex to stop the subagent"). Reliability caveats per research
    §2.6 (orphaned subagent processes; interrupt does not always
    terminate child processes; issue #19197 / #1215 / #7985).
    Authoritative full text and Claude-side / Gemini-side enforcement
    notes: pack-root `CLAUDE.md` `## Pack memory` `### Agent
    invocation rules` "Pack-coder PREFLIGHT + STOP-MEANS-STOP
    pattern" bullet.
...
- **Batch-scope claims are enforced by CI, not honor system.** When
  ...
  defined in § "Rules for Codex agents working on this repo" →
  commit-subject scope-keyword convention.
