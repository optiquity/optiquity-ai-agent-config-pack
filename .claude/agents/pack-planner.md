...
path, your final action MUST land the plan at that exact path — with the
Write tool (or a chunked Edit sequence), or with a Bash heredoc /
redirect to that exact path when the harness steers you toward Bash; the
artifact at the path is the deliverable and the tool used is immaterial.
Emitting the plan as a chat message in lieu of the file is a defect.
