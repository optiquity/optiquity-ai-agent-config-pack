...
**BD-136 — Trinity marker-section preservation pattern (CLAUDE.md / AGENTS.md / GEMINI.md) + PM-chat authoring procedure**
Type: TODO(version) — surfaced 2026-05-10 during OT v10→v11 trinity prep; verified scope against `scripts/lib/customization-preserve.sh:145-179` (12-class file inventory) and `supporting-docs/MERGE-STRATEGY.md`
Status: Open
Blockers: none (independent of remaining v11.0 batches; can land any time before Batch 22 BD-102 dog-food migration)
Unblocks: byte-identical preservation of project-customized prose inside trinity files across pack updates; eliminates the OT-style "one-shot manual re-merge after every pack refresh" burden; closes the explicit BD-088/MERGE-STRATEGY.md §1 placeholder ("Future BDs may add explicit marker-section + diff-recognition fallback")
File/Symbol:
  - `project-template/CLAUDE.md`, `project-template/AGENTS.md`, `project-template/GEMINI.md` — wrap the existing `## Project addenda` H2 body in a seed `<!-- BEGIN project-owned --> ... <!-- END project-owned -->` marker pair (markers around content body only, NOT around the H2 header)
  - `scripts/lib/customization-preserve.sh` — extend (or add sibling `scripts/lib/marker-preserve.sh`) so trinity class detects marker pairs first and preserves in-marker content byte-identical, falling back to existing 3-way text merge for outside-marker content. Multiple marker pairs per file MUST be supported (Option B "in-place markers" — anchors customizations under their semantically-relevant H2/H3, not in a single addenda dump). Unclosed/orphaned markers MUST fail loud (not silent). Concrete merger requirements derived from OT prep verification (`maintenance-docs/v11-implementation/PACK-REVIEW-OT-TRINITY-PREP.md` §5):
      - **L-1.** Marker pair MUST NOT enclose an H2 or H3 line (BEGIN immediately preceding `^#{1,3} ` is a defect — validator catches).
      - **L-2.** Whole-body wrap is a supported pattern (project wraps everything between H2 and next H2). When canonical adds new content above the project's marker pair, merger MUST emit a sidecar warning ("canonical added new pack content above your marker pair under `## <heading>` — review and either delete or fold into your wrap").
      - **L-3.** Fill-in placeholders are a third class beyond pack-owned and project-owned (Class B): `**Active skills:**`, `[PROJECT_NAME]`, `[PLATFORM_DEFAULTS]`, `[PLATFORM_TARGETS]`, `[TRANSPORT]`, `[PLATFORM_TESTING]`, `[PLATFORM_ARCHITECTURE]`, `[LANGUAGE_RULES]`, `[GRPC_RULES]`, `[PLATFORM_SECURITY]`, `[PLATFORM_ANTIPATTERNS]`, `[CONDITIONAL]` H2 prefixes. Spec MUST explicitly resolve via either (a) procedural — PM-CHAT.md procedure requires every fill-in be wrapped inline in single-line markers (RECOMMENDED — no new merger logic); or (b) syntactic — introduce `<!-- FILL: name -->` syntax. (a) is the default; (b) is a future BD if (a) proves too verbose. Without explicit handling, fill-in values silently revert to placeholders on every update (real OT defect: `**Active skills:** apple-architecture-core, …` would have been lost).
      - **L-4.** H2/H3 ownership is asymmetric — pack owns names; project owns body under each. When project edits an H2 name (e.g., dropping `[CONDITIONAL]` prefix, dropping a trailing word), merger MUST detect and emit sidecar conflict; never silently overwrite, never silently keep.
      - **L-5.** New pack H2 inserted between existing project-owned marker pairs MUST be additive — insert in correct lexical position, leave both surrounding pairs byte-identical. Test required.
      - **L-6.** Orphan/unbalanced markers fail loud on the migrator (already in spec). Validator must also check: BEGIN count == END count; strict alternation; no nesting; BEGIN never on the line immediately following a `^#{1,3} ` heading; END never on the line immediately preceding a `^#{1,3} ` heading.
  ...
  - `project-template/docs/pack/PM-CHAT.md` — three coordinated edits: (1) NEW H2 section "How to add project-owned content to trinity files" with the full authoring procedure: markers around body only, never around pack-owned H2/H3 headers; multiple marker pairs per file allowed and encouraged when customizations belong under different topical headers; never edit pack-owned text outside markers — if pack text needs changing that is a BD against the pack; how to handle nested customization; what to do if the pack ships a new H2 that overlaps an existing project-owned section. (2) UPDATE the file-ownership / edit-permissions table (or its equivalent) so trinity files are listed as "pack-owned with project-owned marker sections — see §How to add project-owned content to trinity files" rather than the current generic "PM chat may edit" entry. (3) UPDATE the PM-chat startup checklist (the section the PM chat reads at session start) to include a one-liner reminder pointing at the new H2, so the rule is surfaced every session, not only when editing. Concrete procedure requirements derived from OT prep verification (`maintenance-docs/v11-implementation/PACK-REVIEW-OT-TRINITY-PREP.md` §5):
      - **P-1.** "Body only, never around an H2 or H3" must be the first rule, with a concrete WRONG/RIGHT bad-example block (OT's prep author read the rule and then violated it 4 times per file — so the procedure must show the anti-pattern explicitly).
      - **P-2.** "Do not edit pack-owned text outside markers" must be enforced procedurally: PM chat (a) re-reads the v11 canonical version of any trinity file before editing, (b) diffs working copy against canonical before any edit, (c) refuses to write edits that fall outside marker pairs unless user explicitly approves "edit pack-owned text" — which the procedure should re-route as "open a BD against the pack."
      - **P-3.** Fill-in placeholders enumerated explicitly (all the patterns listed under L-3) with the wrap-inline-in-single-line-markers requirement (per L-3 option a).
      - **P-4.** Whole-body wrap vs. surgical wrap is a deliberate choice with a decision rule: "if you need to edit > 50% of the section body, whole-body wrap; else surgical wrap of the specific lines." Both shapes documented with examples.
      - **P-5.** H2 customization is not supported by the marker pattern. "If you need to add an H2 the pack does not have, add it under `## Project addenda` (whole-body wrap by design). Do not introduce a new H2 outside the addenda section."
  ...
  - `supporting-docs/INSTALL-PROCEDURES.md` — cross-reference to the PM-CHAT.md procedure section
  - `supporting-docs/MERGE-STRATEGY.md` §1 — replace the "Future BDs may add explicit marker-section…" placeholder with the actual specification (matched marker pairs, body-only wrapping, multi-pair support, orphan-marker failure mode)
  - `scripts/validate-pack.py` — new Check (next available number; current count is 30 per Batch 8/9 work) enforcing the full validator surface derived from OT prep verification:
      ...
      - **V-3.** No marker may sit inside a fenced code block (track triple-backtick state line-by-line; emit error if a marker appears with `inside_fence == True`).
      - **V-4.** The `## Project addenda` H2 MUST exist in each trinity file and MUST contain at least one marker pair (the seed pair from File/Symbol bullet 1).
      - **V-5.** Trinity-symmetry warn (per L-7): emit a warning, not error, if a marker pair count differs across CLAUDE / AGENTS / GEMINI.
  - `scripts/tests/test-customization-preserve-bd136.sh` (NEW) — round-trip tests covering the full BD-136 surface, derived from OT prep verification:
      - **M-1.** Round-trip with N=3 marker pairs across distinct H2 anchors; verify in-marker content byte-identical AND outside-marker pack edits adopted via 3-way merge.
      - **M-2.** Pack update inserts a new canonical H2 between two existing project-owned marker pairs; verify new H2 lands in correct lexical position and both project pairs byte-identical (per L-5).
      - **M-3.** Pack update adds a new canonical bullet at the top of a section the project has whole-body-wrapped; verify merger emits sidecar warning (per L-2) and project content byte-identical.
      - **M-4.** Negative test: file with marker pair enclosing a heading line MUST cause migration to fail loud (per L-1 / V-2).
      - **M-5.** Negative test: file with one orphan BEGIN MUST cause migration to fail loud (per L-6).
      - **M-6.** OT-derived golden fixture: once OT's re-prepped trinity files exist (clean per `PACK-REVIEW-OT-TRINITY-PREP.md` §3 fixes), copy them into `test-fixtures/v11-trinity-marker-prepped/` as a real-world golden example — round-trip migration produces byte-identical OT customizations with zero manual reconciliation.
Description: The trinity files (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`) mix pack-managed prose (capability policy, operating rules, agent inventory) with project-side customizations (stack defaults, project-specific operating notes, platform-specific addenda such as OT's iOS 26 deployment guidance). Today the migrator routes them through the generic 3-way text dispatcher, which on `real-merge-required` writes the new pack template at the destination and saves the project copy as a `.pre-update` / `.v10-customized` sidecar — preserving content but forcing manual re-merge on every pack refresh. The OT v10→v11 trinity-prep exercise (2026-05-10) confirmed real-world use: OT has 12 distinct project-owned sections per file (36 marker pairs total across the three files), each anchored under a semantically-relevant pack-owned H2/H3. The marker-section pattern is already proven in `project-template/docs/pack/PM-CHAT.md` (single `<!-- BEGIN project-owned --> ... <!-- END project-owned -->` pair around the "Additional project documents" section). BD-136 generalizes that proven pattern to trinity. Scope is INTENTIONALLY trinity-only: every other project-modifiable file in `project-template/` is correctly handled by its existing class — structured configs (JSON/TOML/env) by key-merge; reserved-prefix `x-*` files by project-only-by-prefix; `pack-agent` / `pack-script` files by 3-way merge with sidecar fallback; `PACK-FEEDBACK.md` and `README.md` (project-owns-the-body shape) by generic 3-way (acceptable; sidecar preserves project content; manual re-merge burden is rare since these templates change infrequently — separate BD if pain emerges). The PM-chat authoring procedure deliverable is non-negotiable per user requirement (2026-05-10): without it the PM chat will drift the moment it adds a new project-owned section. Procedure must be explicit on body-only-not-header wrapping, multi-pair support, and the do-not-edit-pack-text-outside-markers rule. **OT prep verification (2026-05-10) surfaced a third file class beyond pack-owned and project-owned**: Class B "fill-in placeholder" lines where the pack provides shape (e.g., `**Active skills:** [PM chat fills this in]`) and the project fills the value. Without explicit handling these silently revert to placeholders on every pack update. BD-136 resolves Class B procedurally (PM-CHAT.md procedure requires every fill-in be wrapped inline in single-line markers — see L-3 / P-3 above). The full lessons-learned matrix (L-1..L-7 merger, P-1..P-5 procedure, V-1..V-5 validator, M-1..M-6 tests) is documented in `maintenance-docs/v11-implementation/PACK-REVIEW-OT-TRINITY-PREP.md`.
Resolved:
