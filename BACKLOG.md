...
**Spec — Two H2 ownership shapes:**
  ...
  - **Shape B — Project-owned section (whole H2 or H3).** Marker pair encloses both the heading line (H2 or H3) AND its entire body, ending at the natural section boundary (the next same-or-lower-depth heading). Project owns everything: heading name, body content, structure. Used for THREE cases: (1) brand-new project sections the pack has no analog for; (2) project-renamed `[CONDITIONAL]` sections; (3) project-overrides-pack sections (the override mechanism — same H2 name as a pack section signals "suppress the pack version"). On pack update: byte-identical preservation; if the pack canonical also ships an H2 with the same name, the merger SUPPRESSES the pack version and the project's wrapped section wins.
      ...
      <!-- END project-owned -->
      ```
  - **`[CONDITIONAL]` H2s** are init-time scaffolding only: per `INSTALL-PROCEDURES.md` lines 472-479 the project either KEEPS the section (transitions to Shape B with renamed H2) or DELETES it entirely. After init, no `[CONDITIONAL]` prefix should remain in committed project files.
File/Symbol:
  - `project-template/CLAUDE.md`, `project-template/AGENTS.md`, `project-template/GEMINI.md` — seed `<!-- BEGIN project-owned --> ... <!-- END project-owned -->` marker pair around the `## Project addenda` body content (Shape A — pack owns the `## Project addenda` H2; body is intentionally empty-with-marker-pair as the seed slot for project additions).
  - `scripts/lib/customization-preserve.sh` — extend (or add sibling `scripts/lib/marker-preserve.sh`) implementing Shape A + Shape B detection and preservation. Multiple marker pairs per file MUST be supported. Unclosed/orphaned markers MUST fail loud (not silent). Concrete merger requirements:
      ...
      - **L-4 (override mechanism).** No H2/H3 name may appear at most once per file across both Shape A and Shape B in the same file (no duplicates). When the project has a Shape B section with the same H2 name as a pack canonical H2, the merger SUPPRESSES the pack version and the project's wrapped section wins — this is the documented override mechanism. When a Shape B H2 name does NOT match any pack canonical, it is a project-original section.
      ...
      - **L-9.** `[CONDITIONAL]` H2 handling: at init time and at migration time, the migrator MUST treat any `[CONDITIONAL]` H2 in the project file as either kept-and-transitioned-to-Shape-B (project rename + wrap entire H2+body in markers) or deleted (section absent from project file). The literal `[CONDITIONAL]` prefix MUST NOT appear in any committed project trinity file. Validator catches.
  ...
  - `supporting-docs/INSTALL-PROCEDURES.md` — (a) cross-reference to the PM-CHAT.md procedure section, (b) update lines 472-479 (the `[CONDITIONAL]` H2 convention) to reference Shape B as the canonical post-init state for kept `[CONDITIONAL]` sections.
  ...
  - `scripts/validate-pack.py` — new Check (next available number; current count is 30 per Batch 8/9 work) enforcing the full validator surface:
      ...
      - **V-7.** No `[CONDITIONAL]` prefix may appear on any H2 in `project-template/` trinity files in the seed pack templates either — the canonical pack ships them with `[CONDITIONAL]` as init-time scaffolding ONLY in the v9.3-era trinity templates; the v11 seed templates should not (per L-9). Validator enforces in the pack repo's own `project-template/` files.
