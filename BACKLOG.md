...
File/Symbol: `test-fixtures/build.sh` `FIXTURE_NAMES` array (add `v11-realistic-ot`); `_build_realistic_for_version` v11 case body (validate C2 ollama-strip path — `[[ -f ]]` guard at line 226 catches missing v11 file; validate C3 x-agent payload writes correctly to v11's `.codex/agents/`, `.claude/agents/`, `.gemini/agents/` dirs per the `migrator_target_surface_for_version v11` enumeration in `scripts/lib/migrator-core.sh` lines 459-513); `test-fixtures/manifest.txt` (regenerate for new fixture); `test-fixtures/README.md` (table row for `v11-realistic-ot`); possibly extend `scripts/persona-contracts/contract-migration.sh` to v11→v12 dry-run once a v12 migrator exists — likely deferred
