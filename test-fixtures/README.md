...
| `v10-realistic-ot` | v10-pinned | pack at `v10` tag | Fake-OT shape: project-name fills (`FakeOT`); x-prefixed custom agent on Claude/Codex/Gemini; `.codex/config.toml` `model_providers.ollama` removed; 5-entry TD-* `BACKLOG.md`; trinity v10 self-label intact; docs/project monolith trio (`BACKLOG.md`, underscore-spelled `IMPLEMENTATION_PLAN.md`, `CHANGELOG.md`) carrying the migration hazard shapes — non-entry `##` sections with content, a mid-file `# Milestone` H1 divider, a fenced `##` line inside a TD body, a same-date same-phase changelog pair, one out-of-enum `Type:` payload, phases without `Status:` (one without `Goal:`), `Context:` on every TD | Realistic OT-style migration test; exercises BD-088 customization-preservation against shapes a real client would have, plus the per-entry decompose pipeline (rename → accounting gate → synthesis → TOC/index → MIGRATION-TRIAGE) |
...
| `existing-project-collision` | version-agnostic | none — synthesized | Existing-source Swift project (`Package.swift` language marker, `Sources/AcmeTool/main.swift`, top-level `README.md`, `.gitignore`) that already owns a committed, executable `scripts/test.sh` whose content differs from the pack's shipped `project-template/scripts/test.sh`. Contains **zero** pack files (no trinity, no `.claude/`/`.codex/`/`.agents/`), so it classifies as `existing-source`. | Input fixture for the BD-285 C1 existing-source collision-sidecar persona contract: on `init-project.sh` the user's `scripts/test.sh` stays live and the pack version parks at `scripts/test.sh.pack-template`. Version-agnostic — same fixture serves v11, v12, … (BD-285). |
