...
Resolution: 2026-07-13, deprecated per user direction — the fully-autonomous
  execution behavior is now covered by BD-257's operating-mode subsystem (the
  `none` review + `none` intervention pairing, which authorizes auto-commit but
  never auto-push). BD-257's raw-notes descriptions are the source of truth for
  this behavior; a separate `mode: autonomous` CLAUDE.md field and a new
  METHODOLOGY Procedure 8 are no longer needed. BD-039 (Prototype / speed mode)
  is a distinct gate-relaxation concern and remains deferred, outside BD-257 scope.
