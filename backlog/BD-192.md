...
Status: Open
Blockers: v11.0 ships (then the v11.1 cycle architect pass can start); BD-262..BD-268 (the BD-189 children; the umbrella resolved at children-open) should land first so the groupings feature is settled input.
...
File/Symbol:
  ...
  - PRIMARY INPUTS (read-only):
    ...
    - `/backlog/BD-186.md` (Resolved) + `/backlog/BD-189.md` (groupings implementation umbrella) + `/backlog/BD-191.md` (Resolved; PS requirements source-of-truth) for cross-feature and BD-context (repointed 2026-06-13 — `pack-ops/BACKLOG.md` was deleted at BD-203; the `/backlog/` per-entry tree is the sole SSOT)
...
Description: Live forward-pointing anchor for the v11.1+ Product Specialist (PS) core implementation work. All 21 preliminary capabilities of the PS feature (per REQUIREMENTS-PS-V11.md §4) defer to v11.1+ implementation; this BD captures the umbrella work that the v11.1 architect / planner / coder pipeline will break into per-capability BDs.
  ...
  **BD-210 input-classification note (2026-06-13):** the v11.1 architect/planner inputs above include research/handoff docs; BD-210 (pre-launch maintenance-docs cleanup) classifies docs LIVE vs SUPERSEDED and may DELETE some inputs before this BD fires. Treat the input list as LIVE-at-read-time; re-verify each input still exists before consuming it.
