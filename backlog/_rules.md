...
source of truth and readable form. There is no monolithic mirror; do
not recreate one. GH Issues are IGNORED by all tooling; inbound-feedback
issues are a human/PM triage channel only. Validation runs against the
tree. This stream is flat-file per-entry.
...
is the captured `BD-\d+` group only — no letter suffix, no pre-em-dash
parenthetical.
...
explicit BD). Flat-file is the sole supported mode. The write
