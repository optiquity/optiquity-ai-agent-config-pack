...
- `supporting-docs/origins/Claude-Assisted_Project_Methodology_Guide_v1.md` —
  raw source material (the target project's methodology guide, archived for reference)
...
- `.gitignore` (apple, monorepo) — complete Xcode artifact patterns merged from
  the target app: `*.dSYM`, `*.hmap`, `*.ipa`, fastlane, Carthage/Build/
...
- New `## Liskov Substitution Principle` section — generalized from the target project
