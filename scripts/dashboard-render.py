#!/usr/bin/env python3
"""scripts/dashboard-render.py - committed pack-frontier dashboard renderer (BD-224).

The ONE committed realization of the `/pack-dashboard` render contract
(pack-ops/DASHBOARD-SPEC-PACK.md). It authors the presentation (CSS + hash
router + per-view render functions) as versioned string constants, assembles
the COMPLETE `#state` (the 9-field session layer + every backed section + a
live git read), and refuses to emit a board that fails its own complete DATA
floor.

Pack-side ONLY; NEVER shipped to clients (dependency-direction-placement): a
pack operation (`/pack-dashboard`) invokes it, no client surface depends on it,
so it is OFF the `_SANCTIONED_PACK_SIDE_SHIPPED` allowlist. stdlib-only.
...
Two modes (argparse):
  ...
  verify [--repo-root R] [--spec P]  the COMPLETE DATA floor: re-derive the
      ...
      `#state`, and assert Group A (9 session fields), Group B (every backed
      section), Group C (the 4 parse/encoding invariants), and the render-token
      smoke. Exit non-zero with a per-shortfall list on any miss.

`--repo-root` defaults to the script's git toplevel; `--spec` defaults to
pack-ops/DASHBOARD-SPEC-PACK.md - parameters, not hardcoded (keeps the renderer
generic for a later client transplant). Enumerated sets are drawn from
git-tracked files (`git ls-files`), never a raw FS walk; O(lines)/O(one dir),
no whole-tree walk; verify is SKIP-lenient off a git work tree.
"""
import argparse
import json
import os
...
# ---------- layout constants ----------
SPEC_REL_DEFAULT = "pack-ops/DASHBOARD-SPEC-PACK.md"
APPROVALS_REL = "pack-ops/dashboard-approvals"
...
# own defensive sentinel guard `indexOf('__PACK_DASHBOARD_STATE__')` (gap 1).
STATE_SENTINEL = "__PACK_DASHBOARD_STATE__"
...
STATUS_VOCAB = set(STATUS_TOKEN)  # fallback if backlog/_rules.md cannot be read
NEWEST_RESOLVED_N = 10
BODY_MIN, SHINGLE, JACCARD_MIN = 40, 20, 0.30

# ---------- render-surface smoke tokens (measured; ci-guard-measure-then-bound) ----------
# EEB-R1: the render surface is exactly these 10 nav route tokens + 11 p* render
...
RENDER_FN_TOKENS = ["pLanding", "pFrontier", "pGrand", "pBD", "pArchive",
...
_META_FIELD_RE = re.compile(r"^(Type|Status|Target|Blockers|Unblocks|Resolved|"
...
const S=(()=>{try{const t=document.getElementById('state').textContent;
 if(!t||t.indexOf('__PACK_DASHBOARD_STATE__')>-1)return{};return JSON.parse(t);}catch(e){return{};}})();
...
const bd=id=>S.bds&&S.bds[id]||{id:id,num:0,title:id,status:'pending',tier:'minimal'};
...
function sidebar(){
 const g=[];
 g.push(`<div class="brand">&#9670; Pack frontier<span class="v">v${E(S.version||'?')}${S.qualifier?' &#183; '+E(S.qualifier):''}</span></div>`);
 ...
 g.push(`<div class="navgroup"><h4>Work items &#183; ${mo.length}</h4>`+
  mo.map(id=>{const r=bd(id);const st=act.has(id)?'active':r.status;
 ...
 g.push(`<div class="navgroup"><h4>Reference</h4>
  <a href="#archive" data-r="archive">Archive &#183; All BDs</a>
  <a href="#methodology" data-r="methodology">Methodology</a>
  <a href="#rules" data-r="rules">Pack rules</a>
...
function pLanding(){
 ...
 return `<div class="eyebrow">Optiquity AI Agent Config Pack</div><h1>Landing</h1>
 <p class="lede">A live, at-a-glance mirror of pack work &#8212; what is in motion, everything in play, and each backlog item's pipeline.</p>
 ...
 <div class="grid" style="margin-top:6px">
  ${['frontier|Frontier|the running window + in-flight view','grand-plan|Grand plan|everything in play',
     'archive|Archive|the permanent BD index','methodology|Methodology|how work moves',
     'rules|Pack rules|standing rules','help|Help &amp; commands|pack-side commands',
 ...
 <div class="card"><strong>In flight now</strong>${
  (S.active||[]).length?(S.active||[]).map(id=>{const r=bd(id);
...
function sessionBand(){
 ...
 return `<div class="card">
  <div class="kv">
   <div class="k">Active BDs</div><div>${(S.active||[]).length?(S.active||[]).map(id=>`<a href="${idHash(id)}">${E(id)}</a>`).join(', ')+' '+pill('active'):'<span class="muted">none</span>'}</div>
...
function pFrontier(){
 ...
 h+=mo.length?mo.map((id,i)=>{const r=bd(id);const st=act.has(id)?'active':r.status;
 ...
 h+=`<div class="disc">Mirrors pack-ops/session-state.json.</div>`;
 return h;
}
...
function pGrand(){
 ...
 h+=planned.map(id=>{const r=bd(id),pl=plans[id];const pr=pl.progress||{};
 ...
 h+=`<h2>Awaiting planning &#183; ${awaiting.length}</h2><div class="card">${awaiting.map(id=>{const r=bd(id);return `<div class="row"><div class="main"><a href="${idHash(id)}">${E(id)} &#8212; ${E(r.title)}</a></div><div>${pill(S.active.includes(id)?'active':r.status)}</div></div>`;}).join('')||'<div class="muted">None.</div>'}</div>
...
function pArchive(){
 const all=Object.values(S.bds||{});
 let h=`<div class="eyebrow">Permanent index &#183; ${all.length} items</div><h1>Archive &#183; All BDs</h1>
 <div class="card"><strong>Recently resolved</strong>${recentResolved(5)}</div>
...
function pBD(id){
 const r=bd(id);const pl=(S.plans||{})[id];const isAct=(S.active||[]).includes(id);
 const st=isAct?'active':r.status;
 const dec=(S.pendingDecisions||[]).filter(d=>d.indexOf(id)>-1||d.indexOf(id.replace('BD-0','BD-'))>-1);
 ...
 h+=`<h2>Decisions on record</h2><div class="card">${dec.length?dec.map(d=>`<div class="row"><div class="main">${E(d)}</div></div>`).join(''):'<div class="muted">None touching this BD.</div>'}</div>`;
 return h;
}

function pMethodology(){
 let h=`<div class="eyebrow">How work moves</div><h1>Methodology</h1>
 <div class="card"><strong>Large-BD deterministic flow</strong><div class="muted">docs-researcher &#8594; architect &#8594; adversarial architect &#8594; reconciliation (if findings) &#8594; user design review &#8594; planner &#8594; adversarial planner &#8594; reconciliation (if findings) &#8594; user gate &#8594; parallel worktree coder waves.</div></div>
 <div class="card"><strong>Small-BD flow</strong><div class="muted">(optional researcher) &#8594; architect &#8594; planner &#8594; coder + bounded review/fix cycle.</div></div>
...
function pRules(){
 ...
 let h=`<div class="eyebrow">Standing rules &#183; ${rs.length}</div><h1>Pack rules</h1><p class="lede">A live mirror of CLAUDE.md &#167; Pack memory.</p>`;
...
function pHelp(){
 ...
 let h=`<div class="eyebrow">Pack-side commands</div><h1>Help &amp; commands</h1>
 <div class="card"><strong>Pack commands</strong><div class="sec">${cmds.map(c=>`<div class="step"><span class="box">&#183;</span><span class="mono">${E(c)}</span></div>`).join('')||'<div class="muted">&#8212;</div>'}</div></div>
 <div class="proj"><h3>Project-side &#8212; not pack commands</h3><div class="muted">Client project command surfaces are separate from pack-self commands (see BD-257).</div></div>`;
 return h;
}
...
function route(){
 ...
 let html;
 if(/^bd-\d+$/.test(seg)){html=pBD('BD-'+seg.split('-')[1].padStart(3,'0'));}
 else switch(seg){
...
def build_shell(sha):
    ...
    The inert `<script id="state">__PACK_DASHBOARD_STATE__</script>` placeholder
    is replaced (targeted, single-count) by the serialized state at inject time;
    the JS's own `indexOf('__PACK_DASHBOARD_STATE__')` guard uses a DIFFERENT
    lexical context and MUST survive (gap 1). All static glyphs are numeric
    entities so the output is pure ASCII (gap 3).
    """
    prov = ("<!-- pack-dashboard shell &#183; spec: " + SPEC_REL_DEFAULT + " &#183; "
            "spec-sha: " + sha + " -->")
    return (
        "<!DOCTYPE html>\n" + prov + "\n"
        '<html lang="en"><head><meta charset="utf-8">'
        '<meta name="viewport" content="width=device-width, initial-scale=1">'
        "<title>Pack frontier &#8212; Optiquity Config Pack dashboard</title>\n"
...
# FS walk (ci-guard-measure-then-bound). Fail-closed on a broken git ls-files.
# ============================================================================
def normalize(t):
...
def tracked_bd_files(root):
    out = git(root, "ls-files", "backlog/")
    if out.returncode != 0:
        raise SystemExit("git ls-files backlog/ failed (fail-closed)")
    return sorted(Path(root) / rel.strip() for rel in out.stdout.splitlines()
                  if re.match(r"^backlog/BD-\d+\.md$", rel.strip()))


def parse_backlog(root):
    records, failures = {}, []
    for path in tracked_bd_files(root):
        bd_id = path.stem
...
def git_landings_map(root, limit=4000):
    """ONE `git log` pass -> {BD-NNN: [{sha,subject}, ...]} for feat/fix landings.

    A single-subprocess scan (NOT a subprocess-per-BD) so the plans floor stays
    O(one subprocess) - ci-check-runtime-compounding. Buckets every BD-id found
    in each feat/fix subject to normalized `BD-NNN` (>=3-digit) keys.
    """
    ...
    for line in out.stdout.splitlines():
        ...
        if not (low.startswith("feat") or low.startswith("fix")):
            continue
        for num in set(re.findall(r"bd-(\d+)", low)):
            bd_id = "BD-%03d" % int(num)
            landings.setdefault(bd_id, []).append({"sha": sha, "subject": subject})
    return landings
...
def source_anchored_ok(body, title, snippet, src):
    ...
    with the live BD source. Strictly better than no check; defeatable in
...
def load_session(root):
    p = Path(root) / "pack-ops/session-state.json"
    if not p.exists():
        return {}
...
def norm_id(tok):
    m = _BD_ID_RE.match(tok.strip())
    return "BD-%03d" % int(m.group(1)) if m else tok.strip()
...
def parse_rules(root):
    ...
    CLAUDE.md section '## Pack memory' - NEVER index stubs (B4)."""
    p = Path(root) / "CLAUDE.md"
    if not p.exists():
        return []
    lines = p.read_text(encoding="utf-8").splitlines()
    s, e = _section_bounds(lines, "## Pack memory")
    if s is None:
        return []
    rules, group = [], "General"
    i = s + 1
...
# gap 4 / N3: a fully-bold marker line (^**...**$) AND an H3 heading are BOTH
# section boundaries; re-evaluate dormancy at EACH. Non-feature sub-sections
# ('Audit artifacts', 'Carried over to future work') are treated as dormant so
# their bullets are EXCLUDED from the shipped tick list (correctness, not just
# presence).
_DORMANT_RE = re.compile(
    r"DEFERRED|DORMANT|Carried over|future work|not yet ship|Audit artifacts", re.I)


def parse_changelog(root):
    cldir = Path(root) / "changelog"
    panels = []
    if not cldir.exists():
        return panels
    files = sorted((p for p in cldir.glob("v*.md") if not p.name.startswith("_")),
                   key=lambda p: int(re.sub(r"\D", "", p.stem) or 0), reverse=True)
    for p in files:
        lines = p.read_text(encoding="utf-8").splitlines()
        ver, date = p.stem, ""
        for ln in lines:
            m = re.match(r"^##\s+(v[\d.]+)\s+[-\u2014]\s+(.+)$", ln)
            if m:
        ...
        for ln in lines:
            if ln.startswith(">"):
                continue
            s = ln.strip()
            if ln.startswith("### ") or re.match(r"^\*\*.+\*\*$", s):
                dormant = bool(_DORMANT_RE.search(ln))
                continue
            if dormant:
                continue
            if ln.startswith("- "):
                items.append(collapse_ws(ln[2:])[:220])
            elif items and (ln.startswith("  ") or ln.startswith("\t")):
                items[-1] = (items[-1] + " " + collapse_ws(ln))[:320]
        panels.append({"version": ver, "date": date, "items": items[:60]})
    return panels


def parse_agents_roster(root):
    p = Path(root) / "pack-ops/PACK-AGENTS.md"
    if not p.exists():
        return []
    text = p.read_text(encoding="utf-8")
    rows = []
    for m in re.finditer(r"^\|\s*`(pack-[\w-]+)`\s*\|\s*(RO|RW)\s*\|\s*(.+?)\s*\|",
                         text, re.M):
        rows.append({"name": m.group(1), "cls": m.group(2),
                     "role": collapse_ws(m.group(3))[:180]})
    return rows


def parse_help(root):
    """POPULATED help commands from pack-ops/HELP-FRAGMENT-PACK.md verb tables.
    ...
    """
    p = Path(root) / "pack-ops/HELP-FRAGMENT-PACK.md"
    if not p.exists():
        return {"commands": []}
    text = p.read_text(encoding="utf-8")
    cmds = []
...
def help_source_present(root):
    return (Path(root) / "pack-ops/HELP-FRAGMENT-PACK.md").exists()


def status_vocab(root):
    """The legal Status vocabulary - parsed from backlog/_rules.md when present,
    else the STATUS_VOCAB fallback constant."""
    p = Path(root) / "backlog/_rules.md"
    if not p.exists():
...
def assemble_state(root, spec_rel):
    ...
    for bd_id, r in records.items():
        ...
        rec = {"id": r["id"], "num": r["num"], "title": r["title"],
               "status": "active" if bd_id in da else r["token"],
    ...
    for bd_id in sorted(da | newest):
        shas = landings.get(bd_id, [])
        if shas:
            plans[bd_id] = {"sizeTier": "large" if len(shas) > 6 else "small",
    ...
    active_ids = sorted(da, key=lambda x: int(_BD_ID_RE.match(x).group(1)))
...
def inject_state(shell, state_txt):
    ...
    `indexOf('__PACK_DASHBOARD_STATE__')` literal -> SyntaxError -> blank board.
...
def do_build(root, spec_rel):
    ...
    print("RENDER OK")
    print("  spec-sha           : " + sha)
    print("  BDs total          : %d" % state["counts"]["total"])
    n_full = sum(1 for r in state["bds"].values() if r["tier"] == "full")
...
def verify_floor(root, spec_rel, html_path=None):
    ...
    pbds = g(produced, "bds") or {}
    tracked = set(records.keys())
    if set(pbds.keys()) != tracked:  # B1 total-accountability
        dropped = tracked - set(pbds.keys())
        phantom = set(pbds.keys()) - tracked
        errs.append("B1 bds total-accountability: dropped=%s phantom=%s"
                    % (sorted(dropped)[:5], sorted(phantom)[:5]))
    for bd_id, r in records.items():  # B1 status-vocab closure
        if r["status"] not in vocab:
            errs.append("B1 status-vocab: %s has off-vocab Status %r" % (bd_id, r["status"]))
            break
    # B2 |E_full| tier:full with source-anchored bodies (heuristic)
    full_ids = {i for i, r in pbds.items() if isinstance(r, dict) and r.get("tier") == "full"}
    ...
    for i in sorted(full_ids & set(records.keys())):
        r = records[i]
        body = pbds[i].get("body", "")
        if not source_anchored_ok(body, r["title"], r["snippet"], r["src"]):
            errs.append("B2 %s: tier:full body not source-anchored (echo/hollow)" % i)
            break
    # B3 plans floor - every (da | newest) BD with real landings appears in plans
    pplans = g(produced, "plans") or {}
    for bd_id in sorted(da | newest):
        if landings.get(bd_id) and bd_id not in pplans:
            errs.append("B3 plans floor: %s has git-log landings but no plans record" % bd_id)
            break
    ...
    # B5 / C4 changelog v11 panel populated + excludes non-feature markers
    ...
    exp_v11 = next((c for c in expected["changelog"] if c["version"] == "v11"), None)
    prod_v11 = next((c for c in pcl if c.get("version") == "v11"), None)
    if exp_v11 is not None:
        if not exp_v11["items"]:
            errs.append("B5 changelog: FIXTURE/tree v11 panel is empty - floor would be vacuous")
        elif prod_v11 is None or not prod_v11.get("items"):
            errs.append("B5/C4 changelog: v11 panel emptied (dormancy latch) - items=0")
        elif list(prod_v11["items"]) != list(exp_v11["items"]):
            # C4 / N3 correctness: produced items must match the re-derived set
            # (which EXCLUDES non-feature 'Audit artifacts' / 'Carried over').
            errs.append("B5/C4 changelog: v11 items != re-derived set "
                        "(non-feature markers leaked or an item dropped)")
    ...
    # upstream data (deps from each BD's Blockers prose via the B1 `bds` set;
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        prog="dashboard-render.py",
        description="Committed pack-frontier dashboard renderer (build/verify).")
