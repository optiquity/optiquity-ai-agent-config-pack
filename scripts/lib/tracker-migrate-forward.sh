...
tracker_migrate_forward_run() {
    ...
    if [[ "$mirror_only" == "1" ]]; then
        ...
        # `else` branch refreshes the tracker→file read-only mirror
        # HEADER on `$repo_root/BACKLOG.md` — the SEPARATE tracker→file
        # mirror feature (BD-207 owns the project tree EMIT; gated OFF
        # per BD-214), NOT the per-entry→monolith assumption BD-206
        # abolished. It is preserved as-is.
        if [[ "$surface" == "pack" ]]; then
            tracker_error_emit "validation" \
                "forward --mirror-only: mirror-rebuild is not applicable on the no-mirror pack surface — the /backlog per-entry tree is the SSOT (regenerated by the reverse/regen path in tracker mode, NOT a forward mirror-rebuild). BD-203 deleted pack-ops/BACKLOG.md. Run \`pack tracker tree-rebuild\` instead (BD-204 Mode-3 ops contract §2 ride-along (a))."
            return 1
        fi
    ...
    # `pack-ops/BACKLOG.md` to read. The client `else` branch reads the
    # client-root `$repo_root/BACKLOG.md` v10-shape entry-stream INPUT
    # (the legitimate forward-migration INPUT, a v10 monolith being
    # decomposed — EE-11 KEEP — NOT the per-entry→monolith mirror BD-206
    # abolished; the project per-entry tree EMIT is BD-207, gated OFF
    # per BD-214).
