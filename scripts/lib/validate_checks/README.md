...
| `<category>.py` (14 modules) | The check bodies of one connected component — either a cluster of checks that share a non-`core` symbol (plus that cluster's private helpers + constants), OR a single genuinely-isolated check that gets its OWN module per the FIRM own-module-per-new-isolated-check convention (below). |
