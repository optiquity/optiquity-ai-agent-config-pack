...
# invoked by CI [the BD-219 C2 `tests` matrix fixture guard + `tests-result`
# aggregator], by the validate-pack shard-coverage guard [BD-219 C3 Check 60],
# and at MAINTENANCE time as the `include`-block generator [--emit-matrix]).
#
...
# wired-test list; BD-219 C2 FREEZES that generator output into the workflow
# yml as a static `tests`-job `strategy.matrix.include` array, so the wired set
# IS the partition membership list (`wired_set == union(include[].scripts)`).
# parse_wired_tests() reads that `include` array (the SAME parse validate-pack.py
# Check 42 uses), so shard-coverage drift is correct-by-construction: every
# wired KEEP test is assigned to exactly one shard, a non-wired test is never
# assigned, and `--assert-coverage` re-proves it (catching a stale frozen
# `include`).
#
# Realized consumers (architect-doc-reality-reconciliation; never line
# numbers — they drift):
#   - MAINTENANCE TIME: refresh the frozen yml `include` block after a test is
#     added/removed/re-weighted                                 → --emit-matrix
#     (NO LONGER a CI runtime call — there is no dynamic `plan` job; the
#      `include` array is static, generated by re-running this mode and pasting
#      its output into .github/workflows/validate-pack.yml)
...
#
# Inputs:
#   - .github/workflows/validate-pack.yml  (the wired-test list — the static
#                                           `tests`-job matrix.include[].scripts)
...
#   --emit-matrix
#       MAINTENANCE-TIME generator (NOT a CI runtime call — BD-219 C2). Print
#       the GitHub Actions matrix JSON to stdout, one line:
#         {"include":[{"shard":1,"scripts":"a.sh b.sh ..."},...]}
#       Re-run this and paste the output into the static `tests`-job
#       `strategy.matrix.include` block in .github/workflows/validate-pack.yml
#       to REFRESH the frozen partition after a test is added/removed/
#       re-weighted (expand the compact JSON into the yml `include` list shape).
#       The CI guards (Check 42 + --assert-coverage) catch a stale paste.
#   --assert-coverage
...
def parse_wired_tests(workflow_text):
    """Parse the wired test list from the `tests`-job matrix `include` array.

    BD-219 C2 re-anchor: the wired set is now `union(matrix.include[].scripts)`.
    The `tests`-job partition is a STATIC `strategy.matrix.include` array in the
    workflow yml (the FROZEN output of `--emit-matrix`, generated at maintenance
    time), so the wired set IS the partition membership list. There are no more
    per-step `run: bash scripts/…sh` test runners to parse (the run-loop
    `for t in ${{ matrix.scripts }}` executes the shard's `scripts` string).

    Extraction: pull every `scripts/…\\.sh` token that appears inside a
    `scripts:` mapping value under the `include:` block. We match the
    `scripts:` value lines and harvest the `scripts/…\\.sh` tokens from them.
    The `test-fixtures/build.sh` build/verify orchestration is NOT under
    `scripts/`, so the `scripts/` anchor already excludes it.

    This is the SAME wired-set parse validate-pack.py Check 42 (BD-219 C2-
    re-anchored) uses — byte-identical token extraction — so the two never
    disagree on "what is wired" (test-ci-shard-plan.sh Group 6 asserts this).

    Returns a sorted list of unique script paths (repo-relative).
    """
    # Match a `scripts:` mapping value (the per-shard test list), then harvest
    # every scripts/…sh token from that value. Quotes (single/double) and bare
    # space-separated forms are all handled because we tokenize the VALUE for
    # scripts/… paths rather than parsing yaml quoting.
    scripts_line = re.compile(r"^\s*scripts:\s*(.+)$")
    token = re.compile(r"scripts/[^\s\"']+\.sh")
    found = set()
    for line in workflow_text.splitlines():
        m = scripts_line.match(line)
        if not m:
            continue
...
def cmd_emit_matrix(n_shards):
    ...
    # Compact single-line JSON (no spaces) — easy to expand into the static
    # yml `tests`-job matrix.include block at maintenance time (BD-219 C2).
...
def main(argv=None):
    ...
    mode.add_argument("--emit-matrix", action="store_true",
                      help="(maintenance-time) print the GitHub Actions matrix "
                           "JSON to refresh the static yml include block")
