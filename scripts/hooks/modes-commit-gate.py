#!/usr/bin/env python3
# pack-internal: true  (pack-ops intervention_mode PreToolUse[Bash] commit-gate body; never ships to clients, in no install map)
#
# scripts/hooks/modes-commit-gate.py — intervention_mode commit-approval gate.
#
...
# pack-ops/session-config.json, and DENIES the commit unless a FRESH, single-use
# approval token exists (pack-ops/.commit-approval-token). It enforces the
# commit-approval RITUAL (a fresh token must exist per commit) — NOT the
# SEMANTICS (the hook cannot verify the user truly approved; the token is a
# self-attested proxy Pack Chat writes at the approval gate). It is genuine DRIFT
# protection: a commit that skipped the "present approval -> user says yes ->
# write token" ritual also lacks the token, so the omission fails loudly.
#
...
# wedge the human's own commits in EVERY pack-dev clone (this hook is
# auto-wired), so every uncertain branch errs to ALLOW. The deny fires ONLY on a
# fully-resolved {Bash + git commit + cleanly-parsed enforce-mode + no fresh
# token}.
#
# Enforce set: intervention_mode in {full, pre-coder, ambiguity} keeps the
# commit-approval gate (per pack-ops/OPERATING-MODES.md). intervention_mode=none
# authorizes auto-commit -> ALLOW. Any other/absent/malformed value -> ALLOW
# (INERT) — a fresh clone that has not opted into modes must not have its commits
# gated by a token it does not know to write.
#
...
# (single-use — one approval authorizes one commit). Deleting a per-clone runtime
# file is the hook's own bookkeeping, NOT a git verb (agents-never-commit
# untouched). Because consume happens in PreToolUse (before the commit runs), a
# commit that then FAILS has already spent the token -> re-approve; this errs
# toward requiring approval (the fail-safe direction).
#
...
#
# Test seams (mirror install-modes-hook.sh's MODES_HOOK_SETTINGS_FILE seam):
...
# non-`none` values, per OPERATING-MODES.md). Any other value -> allow (inert).
...
def _config_path(cwd):
    ...
    else <toplevel>/pack-ops/session-config.json). "" if unresolvable."""
    ...
    if not root:
        return ""
    return os.path.join(root, "pack-ops", "session-config.json")


def _token_path(cwd):
    """Resolve the approval-token path (test seam MODES_GATE_TOKEN_FILE first,
    else <toplevel>/pack-ops/.commit-approval-token). "" if unresolvable."""
    ...
    if not root:
        return ""
    return os.path.join(root, "pack-ops", ".commit-approval-token")
...
def _deny(mode):
    """Emit the spike-proven deny schema (compact separators, so the substring
    ...
    reason = (
        ...
        "(pack-ops/.commit-approval-token); none present/fresh. Obtain user "
        "approval (which writes the token) before committing."
    ).format(mode=mode)
    out = {
        "hookSpecificOutput": {
