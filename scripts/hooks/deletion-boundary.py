#!/usr/bin/env python3
# pack-internal: true  (pack-ops PreToolUse[Bash] deletion-boundary body; never ships to clients, in no install map)
#
# scripts/hooks/deletion-boundary.py — sub-agent deletion-boundary guard.
#
...
# is defense-in-depth behind the unconditional per-action-approval rule — a
# best-effort backstop for REGISTERED sub-agent spawns, NOT a guarantee.
#
...
# fail-open ALLOW.
#
# FAIL-OPEN at EVERY uncertainty (identical discipline to modes-commit-gate.py):
...
#
# Reads NO session-config.json (the deletion boundary is a universal invariant,
# not a tunable mode).
#
# Test seams (mirror modes-commit-gate.py's MODES_GATE_* seams):
...
# git -c k=v, ...) — reused verbatim from modes-commit-gate.py.
...
def _registry_candidates():
    ...
    if seam:
        return [seam]
    rel = os.path.join("optiquity-pack-handoff", ".pack-agent-owned-dirs.jsonl")
    out = []
...
def _split_segments(cmd):
    ...
    Reused VERBATIM from scripts/hooks/modes-commit-gate.py (the proven matcher).
...
def _deny(target, owned_dir):
    ...
    reason = (
        ...
        "surface it — the orchestrator/harness handles cleanup."
...
def main():
    ...
    try:
        ...
        # main thread (orchestrator) has none -> not a tracked subagent -> ALLOW.
