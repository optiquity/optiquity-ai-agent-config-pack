...
#   7. greenfield empty seed      → docs/project/{backlog,implementation-plan,
#      _toc.md                     changelog}/_toc.md. No monolithic mirror
#                                  (BD-206 no-mirror). BD-166 +
...
s11_files=(
    ...
    # BD-206 (no-mirror): no monolithic mirror at the parent.
...
# BD-206 (no-mirror): no monolithic mirror installs, and _format.md is
# FORBIDDEN on every stream.
for absent in \
    "docs/project/BACKLOG.md" \
    "docs/project/IMPLEMENTATION-PLAN.md" \
    "docs/project/CHANGELOG.md" \
    "docs/project/changelog/_format.md"; do
    if [[ ! -f "$SANDBOX/$absent" ]]; then
        t_pass "S11 artifact $absent ABSENT (BD-206 no-mirror / _format.md forbidden)"
    else
        t_fail "S11 artifact $absent unexpectedly present (BD-206 forbids it)"
    fi
done
