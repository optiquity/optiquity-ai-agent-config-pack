...
| `scripts/init-project.sh` cmd_update | project | MODIFY — put the `groupings/` template files on the install map's `cmd_update` axis so existing clients receive them | `cmd_update` dispatches exactly the rows that the `_CLIENT_INSTALLED_FILES` / `_CLIENT_INSTALLED_GLOBS` blocks carry with the `cmd_update` stage token; the stream needs a family row (or explicit rows) for `project-template/docs/project/groupings/*` on that token. Validation: the customization-preserve classifier must route new files to `generic` 3-way text dispatch per `pack-ops/MERGE-STRATEGY.md`. | The install map is the single enumeration — a file with no `cmd_update` row is not picked up by structure, so the row IS the change. |
