...
| 19 | 18 | smtg-ai/claude-squad | no-op | 2 | 2 | no | tmux + git-worktrees TUI for parallel agents; useful as a personal dev tool but redundant against Claude Code's Agent Teams (already documented in OPTIONAL-FEATURES.md) and breaks the isolation-broken-from-clone caveat already known in v11. |
...
| Client rating | 2 |
| Already in use? | no |
| Integration shape | Overlaps with Claude Code's native Agent Teams (already documented in OPTIONAL-FEATURES.md) and with the pack's existing cross-CLI workflow. Worse, the pack's memory `feedback_worktree_isolation_broken_from_v11_clone.md` documents that worktree isolation already lands under the *main clone's HEAD* rather than the dev-branch worktree's HEAD on Claude Code — that bug would bite harder under claude-squad's per-agent-worktree model. A user could install it as a personal multi-tasking tool unrelated to the pack; the pack does not need to know about it. Not worth an OPTIONAL-FEATURES.md entry. |
| Risks / friction | Worktree-isolation interaction is a real pack bug to keep clear of. tmux dependency. The "no automatic task coordination" property removes the pack's PM-Chat-coordinated discipline; users running claude-squad will fall into the pattern the pack is designed to prevent. |
...
- **claude-squad (18) ↔ pack PM-Chat coordination + known worktree-isolation bug** — wrong layer for the pack's discipline.
