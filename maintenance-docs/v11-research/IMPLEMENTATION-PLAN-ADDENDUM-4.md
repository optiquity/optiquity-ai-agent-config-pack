...
- **§6.P — Architect-default for Path 1 and the `## Phase` body shape. RESOLVED-RATIFIED 2026-05-15 per V3.3-DELTA §7.2 + §3.3 + IPLAN-ADDENDUM-4 §6.P.** Maintainer chose option (a) at BD-107 land-time; PM Chat invokes architect by default for Path 1 (new phase epic). The architect produces the phase shell (Goal, Prerequisite, Risks; possibly initial task list) and explicitly states "planner pass needed" or "no planner pass needed; tasks are self-evident from the TD content," making the planner-invocation trigger an explicit architect-decided downstream call. PM-CHAT.md `## TD resolution orchestration (v11+)` codifies the workflow; METHODOLOGY § Part 7 resolution-path decision logic cites V3.3 §7.2 / §6.P as the basis. Implementation: `scripts/lib/tracker-promote.sh` `tracker_promote_path1` orchestrator; PM Chat handles the architect invocation per the documented workflow. The §6.P MAINTAINER CHECK is now permanently resolved; no further re-evaluation at later BD land-times.

- **§6.Q — Cycle-check K-value. RESOLVED-RATIFIED 2026-05-15 per V3.3-DELTA §5.5 + IPLAN-ADDENDUM-4 §6.Q.** Maintainer chose option (a) at BD-108 land-time; K=10 default, configurable via `tracker.toml [graph] cycle_check_k`. The field is additive (existing tracker.toml schemas continue to load); 10 hops covers V1 §6.1 GraphQL one-shot capacity. Implementation: `scripts/lib/tracker-cycle-check.sh` reads K via `tracker_config_get` with 10 default fallback; the `[graph] cycle_check_k` field is documented in `scripts/lib/tracker-config.sh:30-37`, surfaced as commented blocks in `tracker.toml.pack-example` and `project-template/tracker.toml.project-example` (BD-108 fix coder F12), and mentioned in `OPTIONAL-FEATURES.md` (Batch 17 end-of-batch fix F8). The §6.Q MAINTAINER CHECK is now permanently resolved; no further re-evaluation at later BD land-times.
