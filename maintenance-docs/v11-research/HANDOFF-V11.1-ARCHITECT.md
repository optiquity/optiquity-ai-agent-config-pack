...
- **#13 — Phase-level dependency cycle detection algorithm + persistence choice** (REQUIREMENTS-GROUPINGS-V11.md Capability #13 SC13.22 per §5.1 amendment to PLANNING-PROCESS-INSIGHTS-FROM-OT.md, user-approved 2026-05-24). Validate-pack must detect cycles in the union of member-phase dependency graphs across all groupings; typed error per BD-070. Architect decides: (a) algorithm choice (Tarjan SCC vs alternative — graph size is small; algorithm performance is not load-bearing; choice may be driven by reuse of existing pack graph infrastructure if any); (b) persisted-cache vs rebuild-per-invocation (cycle state has no schema or contract impact; cache is a pure performance optimization with cost-of-staleness considerations). Shared infrastructure with #9 derived-query verbs per SC9.9. Failure-mode anchor: OT PA-013 (7 cycles, 60 of 196 features unlayerable until manual cleanup) — see PLANNING-PROCESS-INSIGHTS-FROM-OT.md §4.2 + §5.1.
- **#4 — `architectural-seam` Kind value consideration** (REQUIREMENTS-GROUPINGS-V11.md Capability #4 default Kind set; per §5.2 amendment candidate in PLANNING-PROCESS-INSIGHTS-FROM-OT.md, deferred to architect-investigation 2026-05-24 rather than adopted at requirements time). Evaluate whether `architectural-seam` should join the default Kind set (expanding from 9 to 10 values) or be left for per-project extension via `_rules.md` per SC4.2. OT evidence supports the pattern's effectiveness — seams are forward-compatibility commitments distinct from `architectural-pattern` (design disciplines like Repository / Factory). Pack-shipping vs project-extends is a defaults-philosophy choice: shipping the value adds it to every client's default vocabulary; deferring to extension keeps the default lean and requires projects to opt in. Future Kind values may also be added during implementation (e.g., research pass surfaces additional empirically-grounded Kinds); architect should consider holistically rather than treating `architectural-seam` in isolation. Reference: `PLANNING-PROCESS-INSIGHTS-FROM-OT.md` §5.2. **Preliminary; subject to architect challenge at v11.1+ groupings design pass** (per pack memory `feedback_preliminary_triage_architect_challenge`).
