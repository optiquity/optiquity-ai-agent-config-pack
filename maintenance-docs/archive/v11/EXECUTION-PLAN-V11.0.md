...
### 1.6 Group 4 — deferred to v11.1+ (9 BDs, NOT in v11.0)
...
## 4. Batch plan (31 batches — refreshed 2026-05-22)
...
> **2026-05-22 mechanical refresh.** Added rows reflecting BDs that
> landed in BACKLOG between the prior §4 update and HEAD `cbbce43`
> (worktree HEAD at refresh time; the request named `9a95bfa` but the
> live HEAD has advanced — refresh applied against live state):
> Batch 19c (BD-173, Open); BD-175 emergency batch (BD-175 → BD-184, all
> Resolved 2026-05-21); Batch 19d (BD-185, Open); Batch 19d-parallel
> (BD-186, Open); Batch 23 row updated to include BD-174 (Open) as the
> first leg of the live-GH test trio per BD-174's declared position.
> Mechanical reflection only — no new sequencing decisions made. BDs
> without declared batch positions (BD-172, BD-187) surfaced in §6 for
> Pack Chat triage rather than slotted. The existing Batch 19 row's
> internal sub-letter commit-label scheme (19a/19b-pack/…/19g-pack)
> collides naming-wise with the separately-numbered Batches
> 19c/19d/19d-parallel rows added by this refresh; surfaced in §6 for
> Pack Chat resolution.
...
| **19** | sequential pack-coder + Pack Chat (mixed) | per-entry split (mandatory v11.0): BD-164 → BD-167 → BD-167b → BD-165 → BD-166 → BD-168 → (BD-160 + BD-170 combined per R-2) → BD-169 → BD-169b → status-flips | `scripts/lib/per-entry/*` helpers + `scripts/migrate-v10-to-v11.sh` post-dispatch hook 6th sub-op + `scripts/init-project.sh` stage extension + `scripts/validate-pack.py` Checks 32+33+34 + `project-template/docs/project/<stream>/` canonical templates × 5 streams + pack-* agent prompt edits + trinity Key files line additions (PM-only) + STATUS.md disclaimer guidance in PM-CHAT.md + PACK-AGENTS.md PM-only directories list expansion (PM-only) + READ-site wording updates + audit-methodology SKILL.md scope extension | After Batch 18 (BD-111 dependency-API switch — final tracker surface dependency); BD-161 absorbed into BD-167; BD-160 combined with BD-170 in commit 19f per Pack-Chat-direct R-2 resolution. 10 commits. Mixed mode: pack-coder commits 19a/19b-pack/19c/19d/19e/19f/19g-pack + Pack Chat direct commits 19b-PM/19g-PM/19h. Per `maintenance-docs/v11-implementation/PLAN-PER-ENTRY-SPLIT-BATCH-19.md`. **Naming note:** the internal commit sub-letters here (19a/19b/19c/…/19g) are commit labels within Batch 19, NOT separate batch numbers. Batches 19c/19d/19d-parallel below are SEPARATE batches that re-use the 19x letter space — see §6 collision observation. |
| **BD-175 emergency batch** | mixed pack-architect + pack-coder + pack-reviewer + Pack Chat | BD-175 → BD-176 → BD-177 → BD-178 → BD-179 → BD-180 → BD-181 → BD-182 → BD-183 → BD-184 (all **Resolved 2026-05-21**) | pack/project boundary audit + re-architecture; RC9 manifest-regen trigger expansion to pack-ops/ + supporting-docs/install-to-client; pack-help.sh:86 sentinel/regex coordination; project-template/ trinity asymmetry alignment + Tier 0 base-loading note (POQ-F4-3); validate-pack.py Check 40 (pack-ops/ bare cross-reference scanner); cmd_update mapping symmetry extension to gemini commands + pm-startup skill + .claude settings example + per-entry templates + supporting-docs/install-to-client; Check 18 H2 pack-root trinity parity guard extension; cross-CLI reference normalization across project-template trinity; Check 16 + Check 19 pack-root trinity extension + BD-181 NIT-1 fold-in; Check 42 CI workflow per-check test wiring prevention check | Emergency batch surfaced 2026-05-18 from PATH-C-CURATION walk-through. PAUSED Batch 19c (BD-173) until Resolved. Batch closed 2026-05-21. 10 BDs, all **Resolved 2026-05-21**. Per BD entry text each BD declares the chain "must close successfully in that order." See archived batch artifacts at `maintenance-docs/archive/v11/` per Pattern B sweep. |
| **19c** | architect-led multi-pass + sequential pack-coder × N | BD-173 (**Open**) — project-side cleanup (project-template/ trinity + agents + skills + ops docs consolidation) | `project-template/{CLAUDE,AGENTS,GEMINI}.md`; `project-template/.claude/agents/`, `.codex/agents/`, `.gemini/agents/`; `project-template/skills/`, `.codex/skills/`, `.gemini/commands/`; `project-template/docs/pack/` | Position declared in BD-173 entry: Batch 19c, between Batch 19b (cleanup) and Batch 20 (features). Project-side analog to Batch 19b (pack-side consolidation). Was PAUSED by BD-175 emergency batch (now Resolved); 19c resumes per BD-175 close-out decision. Multi-pass per Batch 19b precedent: user-input collection → architect (first pass) → user review → architect addendum/V2 → user review → planner → user review → pack-coder × N per-commit with inline reviewers → end-of-batch broad reviewer → BD status flip. |
| **19d** | docs-researcher → architect → planner → pack-coder (full pipeline) | BD-185 (**Open**) — phase parts hierarchy + tracker-mode execution ordering | `project-template/.github/ISSUE_TEMPLATE/work-item.yml` (Part field + part:M label namespace); `supporting-docs/METHODOLOGY.md` Part 3 § "Multi-part phases"; `scripts/lib/tracker-provider-*.sh` (bi-directional part membership + execution order sync); `scripts/migrate-v10-to-v11.sh` + any v11.0 flat→tracker migrator (pre-existing whole-number phase pass-through); `scripts/validate-pack.py` (new check(s) for part-membership + ordering invariants); `project-template/docs/pack/PM-CHAT.md`; `project-template/STATUS.md` | Position declared in BD-185 entry: Batch 19d, immediately after Batch 19c (BD-173). Fires after BD-173 closes and commits. Docs-researcher prompt persisted at `maintenance-docs/v11-research/BD-185-DOCS-RESEARCHER-QUEUED-PROMPT.md` (one-time exception to "Pack Chat does not edit maintenance-docs/" rule per user direction 2026-05-21); firing requires explicit user approval. Pipeline: docs-researcher → revised-architect-prompt → architect → user review → planner → user review → coder per pack memory `feedback_researcher_architect_planner_pipeline`. |
| **19d-parallel** | Pack Chat sidecar requirements pass (REQUIREMENTS doc only; downstream architect/planner/coder open as separate BDs later) | BD-186 (**Open**) — groupings requirements + v11.0/v11.1 scope decision | NEW `maintenance-docs/v11-research/REQUIREMENTS-GROUPINGS-V11.md`; INPUTS (read-only): `maintenance-docs/v11-research/TOUCH-POINT-INVENTORY-GROUPINGS-V2.md`, `maintenance-docs/v11-research/V11.1-DISCUSSION-GITHUB-PROJECTS.md`, `pack-ops/BACKLOG.md` BD-185 entry | Position declared in BD-186 entry: parallel to Batch 19d (BD-185). Independence verified — phase identifier grammar `phase-N` stable across BD-185 SC3; user constraint C1 excludes phase parts from grouping membership; tracker-side surfaces do not collide. No hard sequencing dependency in either direction; downstream grouping architect can run before/after BD-185 architect as scheduling permits. |
...
| **20.5** | sequential pack-coder | BD-136 implementation (4 sub-commits) | (1) marker-aware merger in `scripts/lib/customization-preserve.sh` (or new sibling `marker-preserve.sh`) implementing L-1..L-10 + override mechanism; (2) `scripts/validate-pack.py` new Check enforcing V-1..V-8; (3) PM-CHAT.md authoring procedure section (P-1..P-8) + cross-references in INSTALL-PROCEDURES.md / SETUP-NEW.md / SETUP-EXISTING.md / init-project.sh post-install hint + seed Shape A marker pair in `project-template/{CLAUDE,AGENTS,GEMINI}.md` + `[CONDITIONAL]` retirement in canonical templates; (4) `scripts/tests/test-customization-preserve-bd136.sh` covering M-1..M-10 + add M-11/M-12 fixtures to `test-fixtures/` | Renumbered 2026-05-24 from prior "Batch 21b" → "Batch 20.5" per user direction (multi-commit architect-led; lands BEFORE Batch 21 auditor agents so BD-109/BD-110 can potentially exercise marker-section validation). 4 sub-commits, each independently approve-able under stop-before-commit. M-8 fixture (`test-fixtures/v11-trinity-marker-prepped/`) is the round-trip golden the merger must reproduce byte-identical. Inserted by BD-138 (no v11.1 deferral). |
...
| **21.5** | sequential pack-coder | BD-172 (Gate 2 truthfulness extension — single BD, ~1-2 commits) | `scripts/lib/migrate-v10-to-v11/checkpoint.sh` (3 new verification helpers: `_cp_verify_bd104_rename_outcome`, `_cp_verify_bd035_python_arch_advisory`, `_cp_verify_bd144_capability_advisory`); `scripts/lib/migrate-v10-to-v11/gate-2-phase-a-verify.sh` (wire the 3 helpers); `scripts/tests/test-migrate-v10-to-v11-gates.sh` (9 new test cases — 3 helpers × 3 cases each); plus PACK-REVIEW-BD-101-RETRO MINOR-4 (POQ-1) carry-forward in `scripts/lib/migrate-v10-to-v11/{apply,resume}.sh` reordering `_v10_to_v11_orig_post_report` after Gate 2/3 verdict | Inserted 2026-05-24 per user direction. Lightweight single-BD batch lands BEFORE Batch 22 (BD-100) milestone audit so Gate 2 PASS verdict is truthful when audited and before Batch 23 (BD-102) dog-food. Mirror pattern of existing 8 verification helpers; no new dependencies; no architecture change. |
| **21c** | sequential pack-reviewer + pack-coder per BD | retroactive per-BD review-fix for 13 BDs from prior multi-BD batches: BD-122 (B1), BD-120/116 (B3), BD-117/118 (B4), BD-129/130 (B8), BD-131/133 (B9), BD-078/079/112 (B11), BD-095/101 (B13) | per-BD scope: each BD's original committed files (file-disjoint groups: A migrator-core BD-131/133, B provider/labels BD-129/130, C validate-pack BD-078/079, D fixtures+build BD-120/116/122, E release-gate+CI BD-117/118, F three-way diff BD-112, G migrate-script BD-095/101) | After Batch 21.5 lands. Per the 2026-05-15 review/fix cycle memory revision: multi-BD batches that shipped before the new "per-BD AND per-batch" pattern need their per-BD review/fix cycles applied retroactively. 13 commits, one per BD (`fix: v11 — BD-NNN retroactive per-BD review-fix (Batch 21c)`). No new BDs opened (in-session fix work per standing fix-all rule). Inserted to give Batch 22 milestone audit a cleaner state. Excluded from scope: single-BD batches (already covered by single-cycle rule), Batch 14 (audit-only — per-BD audits already happened by design), Batches 17/19/20/21 (use new pattern naturally going forward). |
...
| **23** | sequential pack-coder + manual; BD-174 → BD-171 → BD-102 live-GH test trio (per BD-174's declared ordering 2026-05-17) | BD-174 (**Open**) — scratch-pack-clone migration + multi-toggle test harness **→** BD-171 (**Open**) — real-OT scratch-GH-repo migration test fixture + harness **→** BD-102 dog-food migration (BD-136-aware) | BD-174: provision scratch GH repo via `gh repo create`, clone pack repo to scratch, push to scratch, run end-to-end v10→v11.0 migration on scratch clone, full multi-toggle (flat → GH Issues → flat → GH Issues), teardown via `gh repo delete --yes`. BD-171: provision scratch GH repo, clone real OT to `/tmp` at v10.1 tag, push to scratch, run migration end-to-end, verify preservation invariants, teardown. BD-102: run `migrate-v10-to-v11.sh` against pack-repo clone at v10 tag; verify clean output, customization preserved. **MUST exercise the BD-136 marker-aware merge path** (per BD-138 scheduling): pack-repo trinity files prepped with Shape A + Shape B markers BEFORE the dog-food run; round-trip preserves all marker content byte-identical. Real OT repo is read-only; no destructive operations on the source. | Produces dog-food report under `maintenance-docs/v11-implementation/DOG-FOOD-MIGRATION-REPORT.md` (BD-102), real-OT migration report under `maintenance-docs/v11-implementation/REAL-OT-MIGRATION-REPORT.md` (BD-171), and scratch-pack-clone migration report under `maintenance-docs/v11-implementation/SCRATCH-PACK-CLONE-MIGRATION-REPORT.md` (BD-174). Per BD-174's ordering rationale: BD-174 first (catches code bugs in safe env, pack-on-pack), BD-171 second (validates client surface, pack-on-OT-content), BD-102 last (final ship-decision gate on real pack repo). Per the in-session fix rule (§B), Pack Chat reports all defects to the user, presents fix options per defect (including NITs), asks permission, and ships the fixes in the same batch (or in a Pack-Chat-approved follow-up commit). No new BDs are opened for dog-food or real-OT findings. |
...
**Total: 32 main batches (24 + Batch 5b for BD-135 + Batch 20.5 for BD-136 implementation (renumbered 2026-05-24 from prior "Batch 21b") + Batch 21.5 for BD-172 Gate 2 truthfulness extension (added 2026-05-24) + Batch 21c for retroactive per-BD review-fix per 2026-05-15 memory revision + BD-175 emergency batch + Batch 19c + Batch 19d + Batch 19d-parallel per the 2026-05-22 refresh) + up to 3 conditional in-session fix commits if audits/dog-food surface defects = max 35 commits, plus Batch 20.5 internally ships 4 commits AND Batch 19 ships 10 commits internally per the per-entry split BD-split per ARCHITECTURE-PER-ENTRY-SPLIT-INTEGRATION.md §17.3 + ADDENDUM §6.5 AND Batch 21c ships 13 commits (one per BD) AND BD-175 emergency batch shipped 10 BDs across many commits (closed 2026-05-21).** Could be slightly higher if any audit / dog-food gate needs more than one small follow-up commit. **2026-05-24 refresh disclaimer:** the 32-batch arithmetic above is a mechanical reflection of the §4 row count after the 2026-05-24 refresh (Batch 21b → 20.5 rename + new Batch 21.5 for BD-172); the BD-175 emergency batch row consolidates 10 BDs into a single row by design (the batch is closed); Batches 19c / 19d / 19d-parallel may need re-sequencing per Pack Chat triage of the batch-numbering collision surfaced in §6.
