...
### 3.LF.5 C-4.6 — the CI faithfulness guard (RUNTIME-CORRECTED per §4.6/§4.7; review verdict PREVENTED)

> **Runtime-corrected (design §4.6/§4.6.1/§4.7; non-recurrence gate `DESIGN-REVIEW-BD-204-C46-RUNTIME.md` = PREVENTED).** The first C-4.6 implementation hung the battery 1.5h+ because the heavy whole-real-tree scan ran inside the 151× general `main()`, ignored the caller's tree (`tree_dir or REPO_ROOT/backlog`), and spawned ~2,000 subprocesses/run (codified in `feedback_ci_check_runtime_compounding`: cost = per-run × battery-invocation-count). The 4 CORRECTNESS legs + the byte-faithful assertion + the no-drop-allowlist + OQ-4 (drive the REAL functions) are UNCHANGED; only PLACEMENT, TARGET-TREE SCOPING, the BATCH SEAM, and a new RUNTIME-BUDGET GUARD are corrected below. The two review SHOULDs (SHOULD-1 both deep homes + impl-reviewer confirms the REAL ≥211 tree runs; SHOULD-2 the 10s total-run FAIL is GENERAL-PATH-ONLY, the deep run gets its own larger total budget) are folded in.

- **Commit subject:** `feat: v11 — BD-204 check_migrator_field_faithfulness (deep-gated) + run_check runtime-budget guard (pack-only)`
- **File scope (by path + symbol):**
  - `scripts/validate-pack.py` — (a) NEW `check_migrator_field_faithfulness(tree_dir)` (the NEXT FREE registry integer — 49 today; coder reads the registry, NEVER hardcodes) + its registration in `main()`; (b) the `run_check(name, fn, budget_s)` TIMING HARNESS wrapping every check + the total-run budget enforcement (§4.7).
    - **ENV-GATE PLACEMENT (§4.6 (P); the load-bearing fix):** the check's heavy whole-real-tree leg runs ONLY when `PACK_VALIDATE_DEEP=1`. In the DEFAULT (unset) general path — the 151× battery path AND ordinary `python3 scripts/validate-pack.py` — the check is a ONE-LINE early-return NO-OP: it prints `SKIP: field-faithfulness deep check (set PACK_VALIDATE_DEEP=1)` and returns BEFORE any tree scan (one `os.environ.get` + print + return ≈ 0 ms). The env check is the FIRST statement in the check body, before any directory read. The general `main()` increment is ~0 (measured baseline 1.34-1.37s stays unchanged across the battery's 151 invocations).
    - **TARGET-TREE SCOPING (§4.6 (T); the exact C-4.6 bug, forbidden):** the check signature takes `tree_dir` = the CALLER's target. There is **NO `tree_dir or REPO_ROOT/"backlog"` fallback** (that `or` silently reverted to the real 211 — the exact bug). A fixture caller pays only its small-fixture cost; the deep CI home passes the real `REPO_ROOT/backlog`. Impl-reviewer grep gate: `tree_dir or` / `REPO_ROOT.*backlog` in the new check body = REJECT.
    - **BATCH SEAM (§4.6 (S); OQ-4 holds):** the check sources the libs ONCE and drives the EXISTING real batch entry-point `tmf_parse_backlog_tree` (`scripts/lib/tracker-migrate-forward.sh:588` — enumerates the stream via `pe_list_entry_files`, concatenates all bodies, parses with ONE `_tmf_parse_backlog_file` call: one `python3` over all entries), then runs compose/reconstruct/emit + the 4 legs in ONE sub-invocation looping over the parsed array. Per-run subprocess count = a SMALL CONSTANT (~4-6 batch spawns/run regardless of entry count), NOT ~10/entry, NOT ~2,000. This is a BATCH DRIVER over the SAME real functions the migration uses (`tmf_parse_backlog_tree` / `_tmf_parse_backlog_file` / `tmf_compose_issue_body` / `tracker_migrate_reverse_reconstruct` / `_tmr_emit_pack_tree`) — NOT a codec re-impl, so OQ-4 holds (no second gzip+base64; a re-impl could drift and FALSE-PASS a lossy migration). `tmf_parse_backlog_tree` already errors (not defaults) on a missing dir, so the seam itself honors target-scoping.
    - **THE 4 CORRECTNESS LEGS (UNCHANGED; approved; no-drop-allowlist):** per entry, inside the one batch process: (1) byte-faithful gz64 round-trip — snapshot the ORIGINAL body span (lines 2..EOF, `pe_strip_backpointer_stdin`) and assert RECONSTRUCTED == ORIGINAL byte-for-byte (FAIL naming the entry + a unified diff of the first differing lines); (2) SIZE — `len(composed Issue body)` (the SAME composed-body measurement the forward composer + the C-7 size leg use, computed by the real composer) within `provider_body_limit - SAFETY_MARGIN`; (3) TITLE — ID-prefixed bold-header title <= 256 (R-TITLE-1); (4) CONTROL-CHAR — no NUL/CR/C0-other-than-tab/LF in any body (R-BODY-6). NO drop-allowlist — byte-equality, full stop.
    - **RUNTIME-BUDGET GUARD (§4.7; now part of C-4.6 scope — the durable prevention):** `main()` routes EVERY check through `run_check(name, fn, budget_s)` (records `t0 = time.monotonic()` before, `elapsed` after). Policy + budgets (measure-then-bounded, RUNTIME axis):
      - **Per-check budget = 2.0 s -> LOUD WARN** in the general path (`RUNTIME-BUDGET: check '<name>' took <elapsed>s > budget <budget_s>s — investigate before merge`); a slow check must not block unrelated work mid-investigation. (Coder measures the slowest current check and sets ~2× it, never below the measured max.)
      - **TOTAL-RUN budget = 10 s -> HARD FAIL, GENERAL-PATH-ONLY (SHOULD-2):** a general run exceeding 10 s means a check regressed into the general path (the C-4.6 compounding shape) -> CI goes RED (`RUNTIME-BUDGET: validate-pack total <elapsed>s > <total_budget>s`). Bracketing: 1.34-1.37s baseline (~7× margin, no false-positive) vs 2-5 min/run failure (far above 10s, caught). **This 10 s total bound is NOT applied to the deep (`PACK_VALIDATE_DEEP=1`) run** — the deep run carries its own larger TOTAL budget (~35 s = ~5 s general allowance + the 30 s deep leg) so a legitimate deep run is never subject to a budget it would breach.
      - **Deep faithfulness-check budget = 30 s** (the batch deep run is measured <= ~5 s; ~6× headroom catches a per-entry-spawn regression without false-failing the healthy batch). Coder re-measures the full 4-leg deep run and confirms < 30 s with margin.
      - Rationale for a GUARD not just discipline: the C-4.6 failure passed THREE design reviews + a sweep because correctness review does not measure runtime; the total-run hard-FAIL is the structural backstop that makes the next pathologically-slow check un-shippable (the RUNTIME analogue of `ci-guard-measure-then-bound`).
  - `.github/workflows/validate-pack.yml` — **TWO deep homes (SHOULD-1; both required):** (a) WIRE the new per-check test (`bash scripts/tests/test-validate-pack-check-<NN>-field-faithfulness.sh`) — MANDATORY in THIS commit or Check 42 (`check_ci_workflow_wires_per_check_tests`, no exemption) goes RED; the per-check test itself sets `PACK_VALIDATE_DEEP=1` and targets the real tree, so wiring it = the deep leg runs on the real ≥211 tree once per push (this is the Check-42-ENFORCED teeth). (b) ADD a DEDICATED deep step (`PACK_VALIDATE_DEEP=1 python3 scripts/validate-pack.py`) that runs the deep check ONCE on the real `/backlog/` per push (prescribed but NOT independently check-enforced — the coder MUST add it; the impl-reviewer confirms it). The existing general `validate` step (`:97`) stays env-UNSET (default-SKIP — ~0 increment).
  - `scripts/tests/test-validate-pack-check-<NN>-field-faithfulness.sh` (NEW; follow the existing `test-validate-pack-check-NN.sh` naming convention; verify `find . -name '<file>' -not -path './.git/*'` empty before naming) — **sets `PACK_VALIDATE_DEEP=1` AND points the check at the REAL `REPO_ROOT/backlog` (SHOULD-1: a green-but-toothless test that set the env but pointed at a 3-entry fixture is REJECTED — the deep check must execute the real ≥211 tree in this CI home).** Legs: POSITIVE (the real all-211 DEEP byte round-trip PASSES) + NEGATIVE (a synthetic lossy/injecting emit FAILS; a synthetic over-limit body FAILS the size leg; an over-length title FAILS; a control-byte body FAILS) + **a RUNTIME-BUDGET-GUARD leg proving a synthetic slow check trips the total-run hard-FAIL (the §4.7 backstop is itself tested, not just declared).**
  - `test-fixtures/manifest.txt` — regen if non-empty (scripts/ touched).
- **Per-commit verification (FULL unattended battery, 3.LF.9) — with WALL-CLOCK MEASUREMENT (the runtime lesson: MEASURE, never estimate):**
  - `python3 scripts/validate-pack.py` GREEN — and the coder records an ACTUAL `/usr/bin/time -p` measurement proving it is ~baseline (≈1.3-1.4 s; the deep leg SKIPPED in the default path; the 10 s total-run budget not breached).
  - `PACK_VALIDATE_DEEP=1 python3 scripts/validate-pack.py` (or the per-check test) GREEN on the real tree — and the coder records an ACTUAL `/usr/bin/time -p` measurement proving the deep run is SECONDS not minutes (< the 30 s deep budget with margin).
  - The FULL battery (3.LF.9) GREEN incl. Check 42 (per-check test wired) + the new per-check test (incl. its runtime-budget-guard leg) + `test-v11-realistic-ot.sh` (banner-pinning).
  - Coder PREFLIGHT + impl-reviewer BOTH confirm (SHOULD-1) the deep check runs the REAL ≥211 tree (not a fixture) in ≥1 CI home; and the impl-reviewer grep gates: no `tree_dir or`/`REPO_ROOT.*backlog` fallback; the check early-returns under unset `PACK_VALIDATE_DEEP` BEFORE any tree scan; the per-check test sets the env AND targets the real tree; the deep run measures < its deep budget.
  - Manifest regen.
- **Keyword:** `pack-only` (touches `scripts/` + `.github/workflows/` only — no `project-template/`, no `supporting-docs/`).
...
**Runtime note (post-C-4.6; §4.6/§4.7).** The faithfulness deep leg is ENV-GATED (`PACK_VALIDATE_DEEP=1`): it runs in exactly TWO homes ONCE each (the per-check test + the dedicated deep step), NOT on the 151× general battery path — every OTHER battery invocation of `validate-pack.py` hits the default path where the deep check is a ~0 ms early-return SKIP, so the battery's validate-pack total stays ~baseline (≈1.3-1.4 s × 151), NOT hours. The `run_check` runtime-budget guard rides EVERY commit's `validate-pack` run from C-4.6 onward: a general run over the 10 s total budget HARD-FAILS CI (general-path-only — the deep run uses its own ~35 s total / 30 s deep-leg budget). Per the runtime lesson, each commit's verification MEASURES wall-clock (`/usr/bin/time -p`) for BOTH a general run (deep SKIPPED, ~baseline) and the deep run (seconds, not minutes), never estimates.
