...
**Update (BD-236, commit `e1af599`).** Check 57 was subsequently promoted from this flat 8-verb intersection to PER-TIER expected-sets — trinity 8 / defs 13 (the 8 + `add, commit, push, tag, apply`) / launcher 13 (identical to defs; completed by adding `git tag`) — closing the gap where a def or the launcher could silently drop `commit`/`push`/`tag`/`apply` and still pass. The 8-verb intersection above (and its framing in the Check 57 bullet + "Why two, not one" paragraph) is the BD-197 as-built point-in-time record, not the current guard.
