...
- **Stable anchor per rule.** Each imperative line ends with a stable slug pointer `[rationale: <slug>]`; the slug resolves to a `## <slug>` heading in `PACK-MEMORY-RATIONALE.md`. The pointer is mechanical (a slug, not "see the rationale doc somewhere").
...
- **Lock-step bijection (see C3) guarantees the pointer never dangles** — every imperative slug has exactly one rationale entry, so `[rationale: <slug>]` always resolves.
...
**Lock-step enforcement (new check) — the bijection:**
- Parse rule slugs from `CLAUDE.md` `## Pack memory` (each imperative line's `[rationale: <slug>]`).
- Parse `## <slug>` headings from `PACK-MEMORY-RATIONALE.md`.
- **FAIL** if the two sets are not equal (every rule has exactly one rationale entry; every rationale entry maps to exactly one live rule — no orphans either direction).
