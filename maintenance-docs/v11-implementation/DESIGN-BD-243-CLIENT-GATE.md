...
**§C.3 addendum (BD-271, Check 85).** Check 85 (`check_narration_twin_content_parity`, `scripts/lib/validate_checks/session_state.py`) introduces a CONTENT-level comparison between the two gates' narration-pattern lists — on its face this looks like it overturns item 2's "no behavioral parity check — that would be a maintenance trap" verdict. It does not; the two are complementary, not contradictory:

1. **What item 2's "maintenance trap" verdict covers.** WHOLE-GATE runtime BEHAVIORAL re-implementation — re-deriving and comparing the two gates' entire enforcement LOGIC (their control flow, their per-axis checks, their emitted verdicts). That verdict STANDS unchanged. Check 85 does NOT do this: it never re-implements or compares gate behavior, never runs either gate, never asserts the two produce the same verdicts.
2. **What Check 85 actually is.** NARROW DATA parity on ONE co-maintained shared literal — the narration-pattern list both gates carry as a near-identical Python tuple: `_SESSION_STATE_NARRATION_PATTERNS` (`scripts/lib/validate_checks/core.py`) ↔ `_SS_NARRATION_PATTERNS` (`project-template/scripts/validate-docs.sh`, inside its `python3 - <<'PYEOF'` heredoc). It is a static AST comparison of `(regex-source, integer-value flags)` per pattern, bounded to a single data structure, with a sanctioned `bd`↔`td` audience fold (measured-and-bounded to exactly the 2 divergent axes). Different, CHEAP (O(two files) — reads exactly two named files, two `ast.parse` calls, no subprocess, no tree walk), BOUNDED.
3. **Why the distinction holds.** The "trap" item 2 warned of is the unbounded, ever-drifting cost of keeping two full RE-IMPLEMENTATIONS behaviorally identical. A single shared literal has no such unbounded surface — it is exactly the co-maintained-constant case a twin-parity data check (cf. Check 80's doc↔constant twin-bijection check) is designed to make cheap and loud. Check 70 (`check_client_doc_gate_parity`) guards the gate's STRUCTURE (presence/axes/wiring — item 2's own check); Check 85 guards ONE shared DATA literal's content. Neither compares whole-gate behavior; they are COMPLEMENTARY.
