...
- **Files:** `scripts/validate-pack.py` (add `check_boundary_and_spawn_pointer_manifests` — or split per G-A; callsite after Check 45); `pack-ops/.boundary-pointer-manifest.txt` (NEW — the B5 surface→pointer manifest, replacing deleted BOUNDARY §6 prose; folds in the §11.3 PACK-CHAT/BOUNDARY routing pointers per §11.3); `scripts/tests/test-validate-pack-check-46.sh` (NEW); `.github/workflows/validate-pack.yml` (wire it).
- **Changes:** reference-resolution over BOTH manifests (Check 34 pattern): every named surface carries its expected pointer; anti-restate substring scan (the C5 collapse makes this PASS); `.boundary-pointer-manifest.txt` includes the §11.3 routing pointers (PACK-CHAT "File access strategy" 4 pointers; BOUNDARY self-homed; review skill) so they ride the existing reference-resolution — no separate check.
...
| 4e | §11.3 routing pointers; index DROPPED; fold into B5 manifest | C6 (manifest fold) + C8 (pointers) |
