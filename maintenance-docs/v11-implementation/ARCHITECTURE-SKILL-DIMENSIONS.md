...
| `scripts/validate-pack.py` | **Check 21** today validates per-CLI pack-help surface parity, not skill canonical phrases. The agent-canonical-phrase check is **Check 27** (per the file at line 1303). For this redesign, Check 27 needs a new conformance: every agent file's "Skills to load" block must enumerate skills consistent with PLATFORM-SKILLS.md's per-agent assignment for that agent. Add a new **Check 32** (next free) that parses PLATFORM-SKILLS.md tables and verifies (a) every skill in `project-template/skills/` appears in exactly one cell, (b) every skill name referenced in any agent file exists as a SKILL.md directory, (c) intersection-table predicates are syntactically well-formed. |
...
2. **Validator updates** — new Check 32 for skill-cell consistency;
