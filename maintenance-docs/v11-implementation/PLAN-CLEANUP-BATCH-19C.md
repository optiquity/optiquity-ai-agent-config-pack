...
**(α-sliding) reviewer-scope interpretation (per Decision 4 (α-sliding) 2026-05-22; UPDATED 2026-05-24 for H.12/H.13 reorder).** Each INLINE reviewer's scope is the diff from the prior INLINE commit (or H.0 baseline) through end of its own commit. Reviewers therefore cover ALL implementation commits at least once before H.17, not just the INLINE-marked commits. See V2 §J.6 sliding-window refinement paragraph and V2 §I "Net per-commit reviewer breakdown" for the canonical mapping. SKIP-per-commit commits (H.1, H.2, H.3, H.6, H.7, H.12) are explicitly covered by the next INLINE reviewer's sliding window — they are not unreviewed. **Sliding-window mapping summary (post-2026-05-24-reorder execution order: H.13 → H.12 → H.14 → ...):** H.4 covers H.1-H.4 (4 commits); H.5 covers H.5 only; H.9 covers H.6+H.7+H.9 (3 commits; H.8 was removed); H.10/H.11/H.15/H.16 each cover own diff only; **H.13 covers H.13 alone** (the prior INLINE was H.11; H.12 has not yet executed); **H.14 covers H.12+H.14** (the prior INLINE was H.13; the sliding window now sweeps the intervening SKIP-per-commit H.12 commit); H.17 end-of-batch reviewer covers the full H.0 → end of H.16 diff as backstop.
...
**EXECUTION ORDER:** This commit lands AFTER H.13 (per-line fence) per the 2026-05-24 architect-spec gap correction. The PLAN H.N names are PRESERVED (per Pack Chat user direction B2: existing H.13 references in committed BD-190 entry and 6 references in GUARDRAILS-CONTRACT.md would break under renumbering). Commit log will show "Batch 19c.13" landing BEFORE "Batch 19c.12" — this is intentional per the reorder. See `IMPLEMENTATION-REPORT-BD-173-Batch-19c-REORDER-H.12-H.13.md` for STOP-AND-ESCALATE evidence + reorder rationale.
...
4. **Add Group 7 fixture-test cases.** Per GUARDRAILS-CONTRACT.md §3.4 T1-T4 verbatim test cases. Append to `scripts/tests/test-validate-pack-checks-36-37-38.sh` AFTER the Group 6 block (which H.13 added per the 2026-05-24 reorder; Group 6 was previously planned to land AFTER Group 7 under the original ordering, but the reorder restored natural numerical sequencing — Group 6 lands first via H.13, then Group 7 lands via H.12). Numbering inversion noted in original planning is RESOLVED by the 2026-05-24 reorder.
...
**Ordering dependency:** MUST land AFTER H.13 (per-line fence covers 4 dual-surface files so scope expansion ratifies cleanly) AND AFTER H.10 (Cat D detect.sh fixes). Per the 2026-05-24 STOP-AND-ESCALATE evidence (see `IMPLEMENTATION-REPORT-BD-173-Batch-19c-REORDER-H.12-H.13.md`): at HEAD post-H.10, the expanded scope catches **26 NEW Check 37 fails** across 4 dual-surface files (`supporting-docs/METHODOLOGY.md`, `supporting-docs/INSTALL-PROCEDURES.md`, `scripts/lib/detect.sh`, `scripts/pack-help.sh`) — these are LEGITIMATE pack-internal references in functional dual-surface code / pedagogical explanation that the H.13 per-line fence (expanded from 7 to 11 files) wraps. Pre-H.13, the architect's original "Pre-sweep PASS verification" (GUARDRAILS-CONTRACT.md §3.3) anticipated only 2 detect.sh `maintenance-docs/` path-prefix leaks (cleared by H.10); the 26 additional unanticipated leaks REQUIRE H.13's fence to land first. Per GUARDRAILS-CONTRACT.md §3.3 corrected "self-validating change" principle: leak sweep (H.10) + per-line fence (H.13) MUST BOTH clear/cover the existing leaks BEFORE scope expansion (H.12) ratifies the cleaned state.
...
**EXECUTION ORDER:** This commit lands BEFORE H.12 (Guardrail 3 scope expansion) per the 2026-05-24 architect-spec gap correction. The PLAN H.N names are PRESERVED (per Pack Chat user direction B2). Commit log will show "Batch 19c.13" landing BEFORE "Batch 19c.12" — this is intentional. See `IMPLEMENTATION-REPORT-BD-173-Batch-19c-REORDER-H.12-H.13.md` for STOP-AND-ESCALATE evidence + reorder rationale.

**Scope:** Per ARCHITECTURE-V11-GUARDRAILS-CONTRACT.md §2. Replace `_is_legitimate_deny_list_doc()` whole-file exemption with per-line `<!-- DENY-LIST-CONTENT-START -->` / `<!-- DENY-LIST-CONTENT-END -->` fence support. Place fence markers in **11 files** per `_CHECK_37_PER_LINE_FENCE_FILES` enumeration (originally 7 — expanded by 4 dual-surface files per 2026-05-24 reorder). Add Group 6 fixture-test cases.
...
- **NEW — 4 dual-surface files added 2026-05-24 per reorder:**
  - `supporting-docs/METHODOLOGY.md` (fence markers around the legitimate `Pack Chat` role-name explanatory text — pedagogical content about the pack-vs-client distinction; sites at L119, L1561, L1579, L1585, L1587 per `grep -nE "Pack Chat" supporting-docs/METHODOLOGY.md`; coder reads HEAD form, identifies the smallest contiguous block(s) around each LEGITIMATE pack-internal reference, and places fences accordingly — multiple non-overlapping fences supported per §2.5 invariants)
  - `supporting-docs/INSTALL-PROCEDURES.md` (fence markers around legitimate `Pack Chat` references at L301 + L609 per `grep -nE "Pack Chat" supporting-docs/INSTALL-PROCEDURES.md`; pedagogical content directing the user to escalate to Pack Chat when manual procedures encounter blockers)
  - `scripts/lib/detect.sh` (fence markers around the `pack-ops/` path-prefix references in functional code comments at L23, L31, L43; the detect.sh script literally needs to scan `$target/pack-ops/BACKLOG.md` when running in the pack repo per BD-175 dual-surface design)
  - `scripts/pack-help.sh` (fence markers around the `HELP-FRAGMENT-PACK.md` + `pack-ops/` references in functional dual-surface code at L38, L39, L86, L87, L92, L106, L112, L113, L114, L115, L119, L120, L133, L136, L153, L169 — 15 distinct lines; the script runs both in pack repo where `pack-ops/` exists AND in client repos where it doesn't, and the script branches accordingly; the `pack-ops/` and `HELP-FRAGMENT-PACK.md` references cannot be removed without breaking the script)
- `scripts/tests/test-validate-pack-checks-36-37-38.sh` (Group 6 test cases)

**Fence-marker syntax note for shell-script files** (`scripts/lib/detect.sh`, `scripts/pack-help.sh`): the HTML-comment fence syntax `<!-- DENY-LIST-CONTENT-START -->` works in shell scripts as ordinary comment text because validate-pack.py's `_build_fence_skip_lineset()` parser looks for exact marker strings at line level (not Markdown-context-aware). Coder uses the shell `#` comment form preceding the marker so the line is a valid shell comment AND a valid fence marker for the parser: `# <!-- DENY-LIST-CONTENT-START -->`. Per §2.5 invariant "each marker MUST be on its own line (no other text on the line)" — the leading `# ` is shell-comment prefix; the rest of the line is the exact marker string. Coder verifies the parser handles the `# ` prefix correctly (or proposes a parser-adjustment fix-coder commit if not).
...
10. **Add Group 6 fixture-test cases to `scripts/tests/test-validate-pack-checks-36-37-38.sh`.** Per GUARDRAILS-CONTRACT.md §2.6 T1-T8 verbatim — 8 test cases covering balanced/unbalanced/nested/empty/outside-fence/multi-fence scenarios. Append Group 6 after Group 5 at line 289 area. Per the 2026-05-24 reorder (H.13 → H.12 → H.14): H.13 lands FIRST and adds Group 6; H.12 lands later and appends Group 7 AFTER Group 6 (natural numerical ordering — Group 6 + Group 7 land in source-file order matching commit order, no §2.6 vs §3.4 inversion). Sequencing now matches GUARDRAILS-CONTRACT.md §5.1 corrected commit order.
...
**Per-commit reviewer:** REQUIRED INLINE — sliding-window scope per Decision 4 (α-sliding) 2026-05-22. Per 2026-05-24 reorder: covers the diff from the H.11-fix commit (`6e2d406`) through this commit's HEAD — i.e., H.13 alone (since H.13 now runs FIRST in the reordered sequence; H.12 follows). Reviewer's scope is the fence-implementation commit (validate-pack.py helper changes + fence-file content edits across the 11 files). Boundary-sensitive surface for reviewer focus (per V2 §I + §H.13):
- **Fence placement preserves intended exempt-content scope.** Verify each fence pair wraps ONLY the deny-list-enumeration / legitimate-pack-internal-content substring, NOT surrounding instructional prose.
...
- **Dual-surface fence placement (4 new files added 2026-05-24).** Verify fences in `supporting-docs/METHODOLOGY.md`, `supporting-docs/INSTALL-PROCEDURES.md`, `scripts/lib/detect.sh`, `scripts/pack-help.sh` wrap only the legitimate pack-internal references (functional dual-surface code paths + pedagogical `Pack Chat` role-name explanations), NOT surrounding prose/code. Verify the shell-script fence syntax (`# <!-- DENY-LIST-CONTENT-START -->`) is correctly parsed by `_build_fence_skip_lineset()`.
- **Sliding-window — post-H.11-fix coverage.** Reviewer covers everything between `6e2d406` and this commit's HEAD; per 2026-05-24 reorder, H.12 (Guardrail 3 scope expansion) lands AFTER this commit and is reviewed by H.14's INLINE reviewer (next in sliding-window sequence).
...
**Pack-coder PREFLIGHT line shape:** `PREFLIGHT: 13/13 in-scope file edits complete; verification PASS; HEAD <SHA>; about to Write IMPL-REPORT to maintenance-docs/v11-implementation/IMPLEMENTATION-REPORT-BD-173-Batch-19c-H.13.md`

**Ordering dependency:** MUST land AFTER H.10 (Cat F removes the BD-175 self-leak from boundary-investigation/SKILL.md:124). Otherwise the fence would ratify a still-leaking state. Per the 2026-05-24 reorder, MUST land BEFORE H.12 (Guardrail 3 scope expansion) — H.13's expanded 11-file fence covers the 26 dual-surface leaks H.12's scope expansion would otherwise surface. Per GUARDRAILS-CONTRACT.md §5.1 (corrected ordering 2026-05-24).
...
**Per-commit reviewer:** REQUIRED INLINE — sliding-window scope per Decision 4 (α-sliding) 2026-05-22 (UPDATED 2026-05-24 for H.12/H.13 reorder). Covers the diff from the prior INLINE commit (H.13) through this commit's HEAD: **H.12 + H.14** (2 commits; H.12 ran SKIP-per-commit between H.13 and H.14 under the reordered sequence; H.14 INLINE reviewer therefore picks up the intervening H.12 scope-expansion commit). Reviewer's scope explicitly includes the H.12 Guardrail 3 `_PROJECT_SIDE_ROOTS` scope expansion (`_iter_client_installed_files()` helper + Group 7 fixture-test cases) + this commit's Check 43 implementation. Boundary-sensitive surface for reviewer focus (per V2 §I + §H.14):
...
- **Sliding-window — H.12 coverage (post-2026-05-24-reorder).** Verify H.12 Guardrail 3 `_iter_client_installed_files()` helper, `_PROJECT_SIDE_ROOTS` constant replacement / delegation, and Group 7 fixture-test cases are reviewed by the dimensions applicable to that commit (contract §3 fidelity; self-validating-change correctness post-H.13-fence; test coverage completeness; manifest-regen attachment). Per the 2026-05-24 reorder, H.12 lands AFTER H.13 (fence covers the 26 dual-surface leaks); the H.14 INLINE reviewer is the first reviewer to see H.12's diff in the sliding window.
...
| H.13 (executes BEFORE H.12 per 2026-05-24 reorder) | Guardrail 2 per-line fence + Group 6 tests + **11 fenced files** (expanded from 7) | validate-pack.py + 11 fenced files (7 original + 4 dual-surface: METHODOLOGY.md + INSTALL-PROCEDURES.md + detect.sh + pack-help.sh) + scripts/tests/ | YES | **INLINE (sliding from H.11; H.13 alone)** | mixed (none) | `feat: v11 — BD-173 Guardrail 2 — per-line deny-list fence (Check 37 modification + 11 files fenced) (Batch 19c.13)` |
| H.12 (executes AFTER H.13 per 2026-05-24 reorder) | Guardrail 3 `_PROJECT_SIDE_ROOTS` scope expansion + Group 7 tests | validate-pack.py + scripts/tests/test-validate-pack-checks-36-37-38.sh | YES | covered by H.14 (sliding-window picks up the intervening SKIP-per-commit H.12) | `pack-only` | `feat: v11 — BD-173 Guardrail 3 — _PROJECT_SIDE_ROOTS expansion to full client-installed surface (Batch 19c.12)` |
| H.14 | Guardrail 1 Check 43 new check + 13 fixtures + CI wire | validate-pack.py + test-validate-pack-check-43.sh + fixtures/project-side-refs/ + validate-pack.yml | YES | **INLINE (sliding from H.13; covers H.12+H.14)** | mixed (none) | `feat: v11 — BD-173 Guardrail 1 — Check 43 (project-side bare cross-reference scanner; V11 leak-sweep prevention) (Batch 19c.14)` |
...
**Per-commit reviewer breakdown (UPDATED 2026-05-24 for H.12/H.13 reorder):** 9 INLINE sliding-window passes (H.4 covers H.1-H.4; H.5 covers H.5; H.9 covers H.6+H.7+H.9; H.10 covers H.10; H.11 covers H.11; **H.13 covers H.13 alone** (post-reorder H.13 lands BEFORE H.12); **H.14 covers H.12+H.14** (the sliding window picks up the intervening SKIP-per-commit H.12 commit); H.15 covers H.15; H.16 covers H.16) / 6 SKIP-per-commit but covered by sliding window (H.1, H.2, H.3 covered by H.4; H.6, H.7 covered by H.9; **H.12 covered by H.14** under the reordered sequence) / 1 END-OF-BATCH (H.17 backstop over full batch diff). Every implementation commit is reviewed at least once before H.17. Note: V2 §I (still reflecting the pre-reorder mapping H.13 covers H.12+H.13) supersedes by this PLAN entry for the post-reorder execution.
