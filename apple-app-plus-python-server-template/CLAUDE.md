...
- Server language default: Python.
- Server must run on macOS, Linux, and Windows unless a dependency clearly prevents it.
- Keep server business logic platform-neutral.
- Avoid macOS-only shell assumptions in server scripts.
- Prefer typed request and response models, explicit validation, and clear API boundaries.
- Isolate third-party API clients behind adapters.
- Keep shared code between client and server narrow and explicit.
